...
- [ ] **Phase 1-F:戰鬥 MVP(自動打怪)** —— 對齊 PLAN.md 當前主攻 slice 3
  ...
  - ✅ 前置之四(敵人反擊威脅聚合純邏輯——補上戰鬥迴圈的另一向,2026-06-05,對齊 PLAN.md slice 4「戰鬥 MVP」):
    `enemy_field::attack_nearest` 處理「玩家自動打最近那隻」(玩家→敵人),但戰鬥是雙向的——
    `combat::EnemyKind::threat`(敵人每次反擊傷害)至今**無呼叫端**,`vitals.rs` 那塊承接點也只有手動餵單一
    threat 的測試;「附近存活敵人反擊扣玩家血」這段把 enemy_field↔vitals 接起來的純邏輯本身還沒人寫,而**任何
    戰鬥接線都得用到它**(否則 `threat` 只是個算出來沒處套的數字、戰鬥淪為無傷收割的空殼)。在 `EnemyField` 加
    `threat_at(px,py)`:回傳玩家此刻承受的反擊總威脅——所有在 `ATTACK_REACH`(刻意**重用**攻擊範圍常數,單一
    真實來源:你近到能自動打牠,牠就近到能還手)內、**仍存活**的敵人各自 `threat` 相加,被多隻包圍時威脅疊加
    (愈深入曠野敵人愈密就愈危險),給「自動打怪」對等的風險面。`attack_nearest` 是玩家→敵人這一向,本函式湊成
    完整迴圈的敵人→玩家那一向。純查詢、**不改任何敵人狀態**(反擊不消耗敵人,敵人照樣被 `attack_nearest` 打)
    故吃 `&self`;反擊**節奏**(接線層多久把威脅套進 `Vitals::take_damage` 一次)留給伺服器迴圈決定,比照
    `attack_nearest` 的 `power`、`tick` 的 `dt`,cadence 是接線層權責,這層只回「此刻圍著你的敵人有多危險」;
    非有限座標回 0(延續 `attack_nearest` 載入防線)。**這是 1-F 戰鬥純邏輯最後一塊缺口**:此後敵人模型(打/被打)
    、佈置/自動鎖定、玩家生命、反擊威脅四塊齊備,接線輪可純接線不再臨時補純邏輯。`EnemyKind::threat` 從此有了
    呼叫端(更新 combat.rs 兩處「無呼叫端」註解保持誠實)。延續「純函式、無 IO、不碰 ws/遊戲迴圈/廣播 shape、標
    `allow(dead_code)` 待接線」的前置慣例。加 5 個單元測試(範圍外為 0、範圍內多隻威脅相加、範圍外排除、被打倒
