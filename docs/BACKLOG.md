...
- [ ] **Phase 1-A：採集節點與動作**
  ...
  - ✅ 前置(採集節點撒佈幾何純邏輯地基,2026-06-05):`gather.rs` 解了「一個節點怎麼被採/
    重生」,接線還差另一半——「世界裡節點該撒在哪、各是什麼種類」。新增 `src/node_scatter.rs`
    純幾何層 `scatter_nodes()`:用座標雜湊(FNV-1a + splitmix64 混合)在 8×8 粗格上確定性地
    決定每格放不放節點(密度 55%)、格內 jitter 落點、依雜湊挑種類(樹/石/乙太礦),並避開家園核心
    (序號 0 地塊四周留一圈 `home_clearance`,世界中心出生點 (1000,1000) 落在序號 0 地塊內故
    一併罩住)。**完全確定性、無隨機/無狀態**——重啟後節點不亂跳(沿用前端草叢以座標雜湊撒佈的
    路數),也給持久化(接 0-E)穩定的世界基準。常數沿用 `field`/`plots`/`state` 既有來源(單一
    真實來源)。延續 `plots.rs` 的前置慣例:純函式、無 IO、不碰 ws/遊戲迴圈、標 `allow(dead_code)`
    待接線輪才有呼叫端;刻意不在這層處理「節點 vs 之後往外長的 per-player 地塊」重疊(O1/O2 的事)。
    加 7 個單元測試(確定性、非空、全落世界內、無節點在家園淨空圈、出生點確在圈內、三種類都有、
    落點互異),`cargo test` 161 綠、`cargo build`/clippy 無警告、伺服器二進位啟動正常(埠被正式
    服務占用屬預期)。**仍待**:接線(同上一條,屬架構級 live 廣播,留待後續輪/PR)。
