...
struct DiskRow {
    ...
    #[serde(default)]
    atlas: u64,
    /// 天象圖鑑 bitmask（ROADMAP 337）。舊存檔無此欄位讀為 0。
    #[serde(default)]
    skylog: u64,
    /// 累積人氣（ROADMAP 341）。舊存檔無此欄位讀為 0。
    #[serde(default)]
    cheers: u64,
}
...
pub struct Saved {
    ...
    #[serde(default)]
    pub atlas: u64,
    /// 天象圖鑑已目睹天象 bitmask（ROADMAP 337）。舊存檔讀為 0（天象全空、重新蒐集）。
    #[serde(default)]
    pub skylog: u64,
    /// 累積人氣（ROADMAP 341 喝采人氣）。舊存檔讀為 0（人氣全空、重新累積）。
    #[serde(default)]
    pub cheers: u64,
}
...
impl PositionStore {
    ...
    pub fn remember(&self, id: Uuid, x: f32, y: f32, ether: u32, wallet_expansions: u32, exp: u32, masteries: crate::class::Masteries, stats: crate::stat_points::StatPoints, skill_masteries: crate::skill_mastery::SkillMasteries, codex: u64, atlas: u64, skylog: u64, cheers: u64) {
        self.inner.write().unwrap().insert(id, Saved { x, y, ether, wallet_expansions, exp, masteries, stats, skill_masteries, codex, atlas, skylog, cheers });
        self.persist_jsonl();
    }

    /// 批次記住多名玩家（給遊戲迴圈定期快照線上玩家用）：更新 cache 一次。
    pub fn remember_all<I: IntoIterator<Item = (Uuid, f32, f32, u32, u32, u32, crate::class::Masteries, crate::stat_points::StatPoints, crate::skill_mastery::SkillMasteries, u64, u64, u64, u64)>>(&self, items: I) {
        {
            let mut m = self.inner.write().unwrap();
            for (id, x, y, ether, wallet_expansions, exp, masteries, stats, skill_masteries, codex, atlas, skylog, cheers) in items {
                m.insert(id, Saved { x, y, ether, wallet_expansions, exp, masteries, stats, skill_masteries, codex, atlas, skylog, cheers });
            }
        }
        self.persist_jsonl();
    }
    ...
    pub async fn flush_one(&self, id: Uuid, name: &str, species: &str, x: f32, y: f32, ether: u32, wallet_expansions: u32, exp: u32, masteries: crate::class::Masteries, stats: crate::stat_points::StatPoints, skill_masteries: crate::skill_mastery::SkillMasteries, codex: u64, atlas: u64, skylog: u64, cheers: u64) {
        let Backend::Postgres(pool) = &self.backend else {
            return;
        };
        let row = [(id, name.to_string(), species.to_string(), x, y, ether, wallet_expansions, exp, masteries, stats, skill_masteries, codex, atlas, skylog, cheers)];
...
pub type OnlinePlayerRow = (Uuid, String, String, f32, f32, u32, u32, u32, crate::class::Masteries, crate::stat_points::StatPoints, crate::skill_mastery::SkillMasteries, u64, u64, u64, u64);
...
async fn upsert_rows(
    ...
    for (id, name, species, x, y, ether, wallet_expansions, exp, m, s, sk, codex, atlas, skylog, cheers) in rows {
        sqlx::query(
            "INSERT INTO players \
        ...
        .bind(*atlas as i64)
        // 天象圖鑑 bitmask（ROADMAP 337）：同為 BIGINT 欄，以 i64 綁定。
        .bind(*skylog as i64)
        // 累積人氣（ROADMAP 341）：同為 BIGINT 欄，以 i64 綁定（單調遞增的小計數，安全）。
        .bind(*cheers as i64)
        .execute(&mut *tx)
        .await?;
    }
...
async fn load_players_from_db(pool: &PgPool) -> HashMap<Uuid, Saved> {
    ...
    let rows = match sqlx::query(
        "SELECT id, x, y, ether, wallet_expansions, COALESCE(exp, 0) AS exp, \
         ...
         COALESCE(skylog, 0) AS skylog, \
         COALESCE(cheers, 0) AS cheers \
         FROM players",
    )
    .fetch_all(pool)
    .await
    ...
    for r in rows {
        ...
        // 天象圖鑑 bitmask（ROADMAP 337，migration 0028）：同為 BIGINT，i64 解碼後重塑回 u64。
        let skylog: i64 = r.get("skylog");
        // 累積人氣（ROADMAP 341，migration 0029）：同為 BIGINT，i64 解碼後重塑回 u64。
        let cheers: i64 = r.get("cheers");
        ...
        map.insert(
            ...
            Saved {
                ...
                codex: codex as u64,
                atlas: atlas as u64,
                skylog: skylog as u64,
                cheers: cheers as u64,
            },
        );
    }
    map
...
mod tests {
    ...
    fn remember_then_recall_round_trips() {
        let store = PositionStore::in_memory();
        let id = Uuid::new_v4();
        store.remember(id, 10.0, 20.0, 5, 0, 0, crate::class::Masteries::default(), crate::stat_points::StatPoints::default(), crate::skill_mastery::SkillMasteries::default(), 0, 0, 0, 0);
        assert_eq!(
            store.recall(id),
            Some(Saved {
                x: 10.0,
    ...
    fn remember_overwrites_previous_state() {
        let store = PositionStore::in_memory();
        let id = Uuid::new_v4();
        store.remember(id, 10.0, 20.0, 1, 0, 0, crate::class::Masteries::default(), crate::stat_points::StatPoints::default(), crate::skill_mastery::SkillMasteries::default(), 0, 0, 0, 0);
        store.remember(id, 30.0, 40.0, 9, 2, 100, crate::class::Masteries::default(), crate::stat_points::StatPoints::default(), crate::skill_mastery::SkillMasteries::default(), 0, 0, 0, 0);
        assert_eq!(
            store.recall(id),
            Some(Saved {
                x: 30.0,
    ...
    fn recalled_ether_survives_round_trip() {
        let store = PositionStore::in_memory();
        let id = Uuid::new_v4();
        store.remember(id, 0.0, 0.0, 42, 3, 200, crate::class::Masteries::default(), crate::stat_points::StatPoints::default(), crate::skill_mastery::SkillMasteries::default(), 0, 0, 0, 0);
    ...
    fn masteries_persisted_and_recalled() {
        ...
        store.remember(id, 0.0, 0.0, 0, 0, 0, m, crate::stat_points::StatPoints::default(), crate::skill_mastery::SkillMasteries::default(), 0, 0, 0, 0);
    ...
    fn stores_are_independent_per_player() {
        ...
        let a = Uuid::new_v4();
        let b = Uuid::new_v4();
        store.remember(a, 1.0, 1.0, 3, 0, 0, crate::class::Masteries::default(), crate::stat_points::StatPoints::default(), crate::skill_mastery::SkillMasteries::default(), 0, 0, 0, 0);
    ...
    async fn flush_is_noop_without_postgres() {
        ...
        store
            .flush_online(&[(id, "阿巡".into(), "terran".into(), 1.0, 2.0, 3, 0, 0, m, s, sk, 0, 0, 0, 0)])
            .await;
        store.flush_one(id, "阿巡", "terran", 1.0, 2.0, 3, 0, 0, m, s, sk, 0, 0, 0, 0).await;
