...
fn system_prompt(npc: &NpcPersona, rel: &NpcRel, gift_available: bool, gift_stock: u32, world_news: &str, elder_context: &str, player_activity: &str, needs_context: &str, relations_context: &str) -> String {
    ...
    format!(
        "{lore}\n\n{persona}\n\n{imp}\n{stats}{gift}{discount}{player_activity}{world_news}{elder}{needs}{relations}\n\n用繁體中文回話，2 到 3 句，口吻溫暖自然、符合世界觀，絕不跳出角色、不要提到你是 AI 或語言模型。",
        lore = WORLD_LORE,
        persona = npc.persona,
...
pub async fn reply(npc: &NpcPersona, rel: &NpcRel, gift_available: bool, gift_stock: u32, player_msg: &str, world_news: &str, elder_context: &str, player_activity: &str, needs_context: &str, relations_context: &str) -> String {
    if !llm_enabled() {
        return canned_reply(npc);
    }
    match ollama_chat(&system_prompt(npc, rel, gift_available, gift_stock, world_news, elder_context, player_activity, needs_context, relations_context), player_msg).await {
        Some(t) => t,
