...
async fn main() {
    ...
    let (positions, inventories, fields, daynight_store) = match db::connect()
        .await
        .expect("Postgres 連線或 migration 失敗")
    {
        Some(pool) => {
            tracing::info!("Postgres 已連線、migration 已套用；玩家位置/背包/農地/日夜時刻走 DB 持久化");
            ...
            let daynight_store = daynight_store::DayNightStore::from_pool(pool).await;
            (positions, inventories, fields, daynight_store)
        }
        None => {
            tracing::warn!("未設 DATABASE_URL；玩家位置/背包/農地/日夜時刻走 JSONL 退回層（本機/測試模式）");
            (
    ...
    let app_state = AppState::with_stores(positions, inventories, fields, daynight_store);
