...
// 水體游泳深化 v1：下水游泳/潛水手感的純邏輯（浮力/水阻/憋氣曲線），前端以相同常數鏡像實作（ROADMAP 930）。
mod voxel_swim;
...
// 乙太方界·居民思考可觀測基礎設施 A4：思考日誌（誰/第幾拍/附近有無玩家/做了什麼/為什麼/是否冒出新心願）
// + 脈絡優先級排序純函式（玩家在場時玩家層置頂、超長砍寒暄社交層），供 M7 接線取用（此模組不碰 voxel_ws.rs）。
mod voxel_think_log;
...
mod voxel_overhear;
// 乙太方界·居民迎新 v1：乙太方界第一套新手引導——全新玩家一進場，最近的居民親自迎接、陪他走完
// 「交談→採集→合成→放置」最初四步，畢業送火把見面禮、把「我迎進世界的旅人」記進她的長期記憶＋
// 動態牆一行；世界的招牌魔法「居民記得你」第一次成為新旅人的第一個體驗（自主提案切片，ROADMAP 948）。
mod voxel_onboard;
...
mod voxel_farm;
// 乙太方界·季限作物·秋南瓜 v1：農業第一次有「只在對的季節種得起來」的作物——秋南瓜只在秋天種得活；
// 與 811/812「四季都能種、時令更划算」正交，開關「能不能種」這一維度（ROADMAP 933·純邏輯季限判定）。
mod voxel_pumpkin;
...
mod voxel_shadow;
// 乙太方界·夜裡點燈守望 v1：入夜見暗影靠近，居民主動朝暗處點起火把、彼此提醒——把被動躲家升級成集體守望純邏輯。
mod voxel_nightwatch;
mod voxel_pettreat; // 寵愛你的夥伴 v1·馴養羈絆線疊了餵食馴服→跟隨→取名→安置召回四刀，卻在「命令」戛然而止，日常裡沒有任何「疼牠」的方式，馴服後再餵食只換來一句冷冰冰的「不用再餵」；本模組把馴服後的餵食死路接成寵愛——遞一份零食（兔一根胡蘿蔔／雞一把種子）換牠蹭你、繞著蹦跳、頭頂浮起愛心，沿用手持食物+準心對準的同一套手勢，羈絆線第一次有了溫度的日常收尾（自主提案切片，ROADMAP 899）
mod voxel_petname; // 為馴服的動物取名 v1·野兔/雞能馴服跟隨卻一直沒有名字（851 自陳「無寵物 UI」）——你親手馴服、每天跟前跟後的小夥伴，和路邊野生動物在畫面上看不出分別；本模組讓你點一下已馴服的小夥伴替牠取名，從此頭頂浮起專屬名牌，馴服→跟隨→繁殖這條羈絆線第一次有了「牠屬於你」的署名（自主提案切片，ROADMAP 895）
...
mod voxel_season;
mod voxel_snowman; // 居民堆雪人 v1·冬天飄雪時閒著的居民偶爾在身旁空地堆一個小雪人（雪塊身＋雪塊頭＋冰燈頂），冒句歡快話、記進心裡與動態牆，冬天一結束就全部融化——季節驅動行為讓世界長出冬天限定的新物件（純記憶體、重啟即消失，自主提案切片）
mod voxel_vigil; // 居民為你留一盞燈 v1·你離線夠久，對你記憶最厚的居民會在夜裡於身旁點一盞火把替你守著；在線的別人夜裡看得見這份牽掛，你在燈還亮著的夜裡回來會被那位居民迎上一句「燈我一直替你留著」並記進記憶，天亮則熄燈——關係（而非季節）驅動世界長出實體發光物件（純記憶體、重啟即消失，自主提案切片 ROADMAP 919）
mod voxel_snow; // 冬季飄雪 v1·冬天下雨即飄雪（前端渲染成白色慢飄雪花，零協議破壞），每個冬天第一次飄雪那一刻附近醒著居民抬頭冒一句「初雪」感言、心情微亮並記進動態牆——把天氣（700）與季節（798）第一次扣在一起的獨立冬季時刻（自主提案切片）
...
// 乙太方界·人生志業 v1：每位居民一條 F5 著墨的「人生志業」——靈魂夢想拆成多階段里程碑，每階映射真實成就，進展由查真實累積狀態湧現得出（零腳本）。純引擎（結構＋trigger 純函式＋進度持久化），不碰 voxel_ws.rs，接線在後續 PR。
...
mod voxel_harvest_stock; // 秋收囤糧過冬 v1：入秋後居民為過冬往你用過的箱子存料更勤、台詞從泛泛「順手存」變成惦記寒冬的「多囤點過冬」，每個秋天第一次囤糧那刻城鎮動態牆留一則季節時刻——把秋收（季節 798）×居民回饋糧倉（chest_contribute）兩套從不相知的系統扣起、給秋天第一個行為後果，與冬雪（900）／圍爐（901）敘事上前後相承（自主提案切片）
...
mod voxel_landclaim; // 玩家個人領地保護 v1：重用告示牌歸屬（830）圈出「家」牌方圓的私有領地，只有立牌本人能挖/放，其他玩家一律溫柔擋下（自主提案切片，ROADMAP 963）
...
mod voxel_lifeproject; // 居民長程自主專案 v1：居民依性格立定一個跨多天的個人大夢（小花園／點燈連路／紀念展台），拆成階段、閒暇一塊塊慢慢蓋、持久化進度跨重啟接續、圓夢時有溫暖收尾，與當下心願／發明互補，自主提案切片
...
mod voxel_request; // 居民會反過來拜託你幫個小忙 v1：夠面熟的居民偶爾主動向你討一樣好採集的基礎材料（木/石/煤/沙），你採來當禮物送到，她特別歡欣道謝並把「你在我開口時幫了我」記進記憶、加深情誼——採集第一次接上居民的需要（記憶→行為·你的互動有後果）；v3 村莊委託牆（自主提案切片，ROADMAP 1015）：全村目前掛著未了請求的居民彙整成一張依剩餘等待時間排序的清單，玩家第一次能主動挑一則最急迫的委託去幫，而非隨緣路過巧遇
...
mod voxel_meteor; // 流星許願 v1：夜裡偶爾一顆流星劃過天際（前端純視覺、伺服器隨快照廣播 meteor:bool，比照彩虹 780），附近醒著的居民抬頭望見而許願——若心裡正懷著一個渴望（voxel_desires 當前心願），許的就是那個藏了好久的願（渴望驅動行為·北極星），與彩虹齊呼(780)／繁星共賞(783)razor-sharp 區隔（自主提案切片 ROADMAP 904）
mod voxel_migration; // 候鳥遷徙 v1：每逢入春一群候鳥飛回、每逢入秋一群候鳥往南飛去（前端單一可重用鳥群 Group，伺服器換季偵測後隨快照廣播 migration:bool＋migration_kind:"arrival"|"departure"，比照流星 904／彩虹 780），附近醒著的居民抬頭望見而心頭一動、城鎮動態留下一筆——把「季節」這條最長時間軸第一次接上一樁看得見的天象事件，與流星(夜)/彩虹(雨後)/換季台詞(地面)razor-sharp 區隔（自主提案切片 ROADMAP 944）
mod voxel_moonphase; // 月有圓缺 v1：繁星夜空(783)的那輪月至今永遠是滿的——本刀讓月亮隨乙太曆盈虧（朔望週期僅 4 遊戲日，幾個夜就看得出圓缺；伺服器隨快照廣播唯讀 moon_illum，前端依受光比例把圓月削成弦月），滿月夜醒著靠近你的居民偶爾抬頭對月出神許願、城鎮動態留一筆「今夜月圓」，與繁星共賞(783,偏好驅動/恆滿月)razor-sharp 區隔（自主提案切片 ROADMAP 946）
mod voxel_mist; // 晨霧 v1：每個清晨整片世界浮起一層薄霧（前端縮短 fog 能見度、日出後漸散，零協議破壞——純用既有廣播的 time_of_day 本地判定），霧起那一刻附近醒著居民抬頭冒一句晨霧感言、心情微亮並記進動態牆——補上從沒碰過的「大氣能見度」軸，與雨天(701)/初雪(900)/彩虹(780)razor-sharp 區隔（自主提案切片 ROADMAP 913）
...
mod voxel_teach;
mod voxel_mastery_fame; // 名匠聲望 v1：把「發明並教過某門手藝夠多次」的居民自然認作村裡該手藝的公認名匠（如「露娜·燒玻璃名匠」）——技能從「會不會」長出「誰是這門手藝的權威」這層，並讓聲望有社會後果：別人卡關優先找名匠、名匠授課稱謂現身、動態牆記下「村裡公認…是一把好手」（純以既有發明/師承紀錄重算、零重複資料、零 migration，ROADMAP 888）
...
mod voxel_longing;
// 乙太方界·兩村相思 v1：殖民地真居住（942）讓拓荒者真的搬離主村後，與老朋友分隔兩座村——本模組讓分屬不同聚落的摯友偶爾在醒著的安靜時刻望向遠方念叨隔村的老朋友（點名＋村落名、上動態牆、記進雙方記憶），因兩邊各自獨立掃到對方而天生雙向往復；與居民惦記玩家（longing·對象是人類）、主村想念（942·搬家那刻一次性泛泛）razor-sharp 區隔（自主提案切片 ROADMAP 945）。
mod voxel_farbond;
// 乙太方界·居民踏出來的小徑 v1：居民日復一日在家/井/工作點之間走同一條路線，草地被踩久了自己踏出一條泥土小徑——小社會的作息第一次寫進世界地面本身（自主提案切片）。
mod voxel_pathwear;
...
mod voxel_nightlife; // 居民夜間生活 v1·入夜後閒著、還沒就寢的居民偶爾抬頭對夜空許個願（無玩家亦發生），或和身邊夥伴在回家前互道晚安——夜晚第一次有了它自己的溫柔日常，和白日的採集建造區隔開（自主提案切片）
mod voxel_nickname; // 居民互取親暱綽號 v1·情誼到老朋友的兩位居民走近時，偶爾為對方取一個親暱綽號（依名字生成，如「露露」「小凜」）當面宣告＋記進動態牆，此後日後相遇會沿用綽號招呼——居民之間第一次以只屬於彼此的暱稱相稱，村子的關係網有了自己的親密紋理（命名→記住→沿用的記憶→行為閉環，自主提案切片）
mod voxel_custom; // 村莊自發習俗 v1（暮聚）·村子豎起中央村碑後，每到黃昏在場閒著的居民便不約而同晃到廣場村碑邊聚著閒話家常、看天色暗下再各自散去——村莊第一個會週期觸發、玩家路過看得見的自發習俗（有文化的小社會第一塊拼圖，自主提案切片）
...
mod voxel_vocation; // 居民生計 v1·每位居民一生不變的日常營生身分（農夫/鐵匠/漁夫/獵人/工匠/商人），閒下來時隨時隨地會安靜忙活一會兒手邊的活兒——補上「她平常都在忙什麼」這個從未被回答的身分軸，與能力(888/889)、私人嗜好(voxel_hobby)三軸並立、正交（自主提案切片，ROADMAP 988）
...
mod voxel_farvisit; // 相思成行·跨村探親 v1·兩村相思（945）的念叨累積滿了，居民不再只是原地惦記——她真的動身走幾百格的遠路去另一座村看那位永久遷居的摯友，重逢、小聚、再踏上歸途：記憶第一次驅動跨聚落的行動（自主提案切片 947）
mod voxel_sendoff; // 為遠行的夥伴送行 v1·居民啟程遠行那一刻，身邊醒著、與牠相識以上的老鄰居會停下手邊的事道一句珍重，兩人交情再深一分——散居·遠行（item 7）第一次在「離開的那一刻」接上居民情誼（item 4），與邊陲探友（821，抵達後跋涉去迎）一送一迎互補（自主提案切片 902）
...
mod voxel_player_bond; // 玩家羈絆帳本 v1·827/828 自己的模組註解都點名「玩家↔玩家至今只有兩三種互動」，但都是瞬間、不留痕跡——本刀讓「常一起協作/常互相轉手」第一次被世界記住：鏡射居民情誼（672）分級精神，兩位真人玩家累積真實互動（協作命中／收下轉手）升級為旅伴/摯友，冷卻防洗刷、升級雙方各跳提示+動態牆記一筆（自主提案切片，接續 984，reviewer 明令離開遠征首領維度，ROADMAP 985）
mod voxel_famine; // 荒年 v1·天氣/天象事件（雨/彩虹/流星/候鳥）皆純氛圍、結束後世界毫無變化，居民餓意（799）純屬個人恆常需求——世界從沒有一樁波及全村、需要互助的集體性稀缺危機。本刀補上：偶爾（低機率+強制冷卻）觸發一段荒年，窗口內全村餓意累積加速，你若在此刻餵飽正餓著的居民，會換來格外不同的深記憶措辭；落幕時把「熬過幾次荒年/這次幾次被及時餵飽」持久記下，成為一份真正累積、不隨重啟歸零的村莊履歷（自主提案切片，換一個全新維度，ROADMAP 986）
mod voxel_elder; // 居民晚年 v1·生命週期軸線蓋到成年禮（942）就戛然而止——長大成人後無論再活過幾個乙太年，永遠只是「成年人」，沒有晚年這一段。本刀補上：世代傳承誕生的居民活過 3 個乙太年步入晚年，一生一次的感言＋含「一定會」的永久精華記憶＋動態牆播報；真實行為後果＝老朋友到訪本可能觸發拌嘴（715），但主人若已是長者，歷經歲月的從容會把這場小摩擦化解成一齣溫暖橋段，生命階段第一次反過來改寫既有互動的走向（自主提案切片，換一個全新維度，接續 986，ROADMAP 987）
mod voxel_stroll; // 摯友結伴同行 v1·居民↔居民關係至今的「一起」全是聚到某定點（711 小圈子/暮聚）或戀愛專屬的奔赴（852 戀人牽掛）——從沒有一刀讓兩位單純的摯友，就只是並肩在村裡走走。本刀補上：白天好天氣裡兩位互為摯友、都閒著且相鄰的居民，偶爾一位邀另一位一起在村裡散步一段，follower 貼著 leader 此刻的位置並肩漫步過村子——小社會的情誼第一次寫進「移動」本身（自主提案切片 925）
mod voxel_walk_with; // 居民邀你散步同行 v1·摯友結伴同行（925）只發生在居民↔居民之間，玩家從不在其中。本刀把「結伴同行」第一次接進玩家：白天好天氣裡，一位對你記憶夠厚（好感達摯友門檻）的居民，見你正閒著就近晃過來邀你「陪我走走」，然後貼著你此刻的座標並肩在村裡漫步一段、記進心裡——玩家↔居民的情誼第一次走在了村子的路上（自主提案切片 926）
...
mod voxel_furniture; // 玩家裝飾傢俱 v1·世界可放方塊至今不是建材就是有用途的功能家具（床能睡/長椅招坐/告示牌寫字/鐘召集），唯獨缺純為好看擺著佈置家園的小傢俱——本刀補上第一批純裝飾傢俱（小地毯/花盆/小圓桌/掛旗）：零互動零居民行為零計時，走矮塊/薄片造型一眼是傢俱而非建材，把「佈置一個屬於自己的角落」這件最療癒的建造樂趣補齊（ROADMAP 931·自主提案切片）
...
mod voxel_nightguard; // 守夜恩人 v1·驅影之劍（887）讓戰鬥成形但驅散暗影至今只換一枚礦、沒社交後果——本模組讓你替近旁受威脅的居民驅散逼近的暗影時，她注意到是你替她解圍、道謝、心情亮一格、記進心裡，把戰鬥（人對怪）第一次接上社交後果（自主提案切片，ROADMAP 888）
mod voxel_siege; // 暗潮之夜 v1·平時暗影只是遠村點綴、村莊從不曾真正被威脅——本模組讓某些夜晚（低機率）暗潮突破村莊庇護圈、集中湧向村莊中心，全村居民一起警醒奔回家躲避、你持劍在燈火間守護大家，天亮暗潮退去全村一起鬆一口氣，把既有零件（劍／光＝庇護／害怕逃家／動態牆）串成一場有起承轉合的集體防禦事件（自主提案切片）
...
mod voxel_worldboss; // 遠征首領 v1·三層戰鬥/抵禦（暗影／驅影之劍／暗潮之夜）全發生在村莊周邊、全是夜間限定、全是多隻小怪——世界始終缺一個生在遠方、恆常存在（不隨黎明消散）、需要主動遠征才找得到的單一首領目標；本模組補上這條「近/夜/被動防守」以外的全新軸線（自主提案切片）
mod voxel_wedding; // 戀人成婚 v1·戀愛弧（心動846→愛巢→牽掛852）的高潮：一對早已是戀人、締結後交情又疊得夠深、白天好天氣正相鄰的居民偶爾就地結為連理——互許終身、鄰居道賀，並在世界裡永久立起一座花拱門作證，成為兩人心裡最重的一筆記憶（一生一次、持久化，自主提案切片，ROADMAP 927）
...
mod voxel_family; // 愛的結晶 v1·927 讓戀人成婚，但村裡的「新居民出生」至今與婚姻無關（只挑一位居民無性繁衍）——本模組讓成婚夫妻第一次「一起」迎來孩子：有夫妻在時下一個新生命由這對夫妻共同誕下、承繼雙方各一點本事、雙親各記一筆「一起守護這個家」的永久記憶，家庭第一次在小社會裡成形（自主提案切片，ROADMAP 928）
mod voxel_sibling; // 手足相伴 v1·928 愛的結晶讓夫妻共同迎來孩子，家庭第一次成形，但只有「父母↔孩子」的縱向一環——同一位父母日後再迎來的孩子，與先來的孩子始終毫無瓜葛；本模組讓新生兒誕生時，若名冊裡查得到同父母的既有孩子，就與那些哥哥姐姐相認為手足（新生兒與兄姊各記一筆含「一定會」的永久精華記憶、兄姊當場轉頭迎接、動態牆以手足口吻播報），家庭最基本的橫向關係「手足」第一次在小社會裡成形（自主提案切片，ROADMAP 941）
mod voxel_coming_of_age; // 長大成人 v1·生命週期缺了「長大」這一環——孩子誕生後永遠是「新生兒」，出生系統挑父母又毫無年齡門檻（剛出生就可能當爸媽）；本模組讓世代傳承誕生的孩子活過整整一個乙太年（一輪四季）就長大成人，行一生一次的成年禮（自己說「我長大成人了」、含「一定會」的永久精華成長記憶、父母記一筆欣慰、動態牆播報），真實後果＝唯有長大成人的居民才會被選為父母，生命週期「出生→童年→成年→成家」第一次真正閉環（自主提案切片，ROADMAP 942）
mod voxel_childhood; // 孩子的模樣與玩耍時光 v1·942 成年禮／987 晚年都只在生命階段「轉換那一刻」鄭重其事，轉捩點之間那段實打實的童年本身卻從沒被世界看見——孩子誕生後體型/動作/日常跟成年居民一模一樣，玩家認不出誰是孩子。本模組讓尚未行成年禮的孩子體型第一次比成年居民小一圈（前端渲染縮放，`is_adult` 單一事實來源、零新持久化），閒晃時偶爾玩起純真的小把戲（追蝴蝶／繞圈圈／踢石子），第一次讓生命階段本身而非只有轉捩點反過來改寫行為與樣貌（自主提案切片，接續 942/987 生命週期軸線）
mod voxel_first_invention; // 第一次發明立碑 v1·真進化（716～867 居民自己發明技能）是這世界的北極星，但「生涯第一次真的靠自己發明」那一刻此前只在泡泡/Feed/記憶閃過、地面看不到痕跡——本模組讓居民首次自己發明成功（自己發明筆數 0→1、繼承/師承的不算）時就地立起一座永久的「發明之光碑」（石磚碑座＋乙太靈光燈），一生一次、持久化，村子各處於是散落著居民們各自「第一次自己想出辦法」的證物（自主提案切片，ROADMAP 930）
...
mod voxel_craft_admire; // 手藝仰慕 v1·名匠聲望（888/889）唯一的社會後果至今只有「卡關優先找名匠來教」，聲望本身從沒有獨立於教學之外的效果——素不相識、還不會這門手藝的居民路過名匠身邊毫無反應。本模組讓還不會某門手藝的居民路過這門手藝的公認名匠時會停下多看兩眼、由衷佩服，情誼因此加深；刻意不設交情門檻（陌生人也會），讓「名聲」本身而非既有交情成為連結居民的新理由，與 800 分食（交情驅動）因果相反、互補（自主提案切片）
mod voxel_pet_fright; // 臨危依偎 v1·馴養羈絆線（851 馴服跟隨／895 取名／898 安置／899 餵零食）至今只活在白天的安寧裡，你取名的小夥伴夜裡撞見漂近的暗影（voxel_shadow）卻毫無反應；本模組讓已馴服的兔／雞夜裡撞見暗影會嚇得竄回你腳邊依偎討庇護、頭頂冒受驚表情，馴養羈絆第一次在危險面前有了意義（寵物 × 暗影首次交會，自主提案切片，ROADMAP 903）
...
mod voxel_envoy_mark; // 引夢使者印記深化 v1·居民早會對玩家生出交情/印象，也有守夜恩人(#888)、收花道謝等零星互動，但從沒把「這位引夢使者為我做過的具體好事」攢成一份自己的記憶時不時回想感念；本模組把守夜驅暗影/送禮/送花等伺服器權威善舉，歸類記進一份持久帳本(向後相容 jsonl)，居民日後平常相處時偶爾回想提起（「還記得那晚你替我趕走暗影…」），讓玩家＝引夢使者真的感覺到「我在這世界留下了印記、而且被記住了」（自主提案切片）
...
mod voxel_settle; // 殖民地真居住 v1·分村殖民奠下的野外村落至今只是空殼（拓荒者只是故事字段）——本模組把「單一村莊中心」漸進一般化成「居民屬於某聚落」：拓荒者真的遷居殖民地（重用 #1145 搬家引擎）、在殖民地小地塊上蓋家生活、孩子承繼父母聚落，第二村第一次真的有人住（架構級大弧 v1，承 PLAN_ETHERVOX §7）
...
mod voxel_caravan; // 跨村商隊 v1·殖民地真居住（943）／兩村相思（945）／跨村探親（947）讓兩座聚落有了人與情感的來往，物資卻至今從沒流通過——本模組讓居民偶爾帶著自己的特產跋涉去另一座聚落做生意、換一批當地貨再踏上歸途，不吃好感度門檻（只看聚落、不看交情），是第一個跨聚落但無關交情的互動，把「散居世界」第一次接上經濟的血脈（自主提案切片，ROADMAP 950，承 PLAN_ETHERVOX §7）
mod voxel_hobby; // 居民私人嗜好與意外驚喜 v1·手藝專精（888/889/956）一路把「她多會這件事」寫得很深，居民卻從沒有一項與能力/聲望無關、純粹是她這個人喜歡什麼的私人品味；本模組讓每位居民有一項固定的私人小嗜好（押花/收藏發光結晶），閒暇攢一點私藏、跟你聊得來時當驚喜送你——不因為你多會她的手藝，只因為她想跟你分享（自主提案切片）
mod voxel_lighthouse; // 眾力共築·乙太燈塔 v1·世界至今每一座值得一看的建物（村碑885/殖民地884/居民個人夢想917/自己蓋的家）全是居民蓋的，玩家彼此之間卻從沒有一件「大家一起出力才蓋得起來」的公共工程——本模組在村外固定選址一座燈塔工地，任何玩家靠近捐出背包裡的建材（石頭/木板/玻璃/鐵磚），全服可見募集進度，材料湊齊即一次性蓋出燈塔並向全世界廣播，是玩家（非居民）第一次共同蓋出一件屬於全世界的建物（自主提案切片，接續 PLAN_ETHERVOX「玩家遊玩」進度/目標軸線）
mod voxel_cohabit; // 邀居同住 v1·830 認得你的家讓居民偶爾登門拜訪，拜訪完照樣走回自己的老家；本模組讓深交（affinity ≥ 教學等級門檻）的居民能被玩家邀請真的搬進玩家登記的家，從此在那附近生活安頓，再邀一次即搬回原本的家——「玩家共居」主軸至今唯一沒兌現的承諾第一次成真（自主提案切片，ROADMAP 972）
mod voxel_dungeon; // 地底遺跡神殿 v1·地底至今只有「天然」的東西（深層礦石/天然洞穴腔室）——世界從沒有一處人工鑿建的地底空間；本模組讓 block_at 世界生成管線在深層石頭裡確定性長出全世界唯一一座入口室—通道—藏寶室的規則神殿，得先挖穿石牆才看得見，找到核心即得一份一次性豐厚獎勵，是「垂直/室內探索」這個維度第一次被打開（自主提案切片，ROADMAP 975，接續 974 遠征首領後換一個全新維度）
mod voxel_busking; // 街頭手風琴 v1·世界至今與音樂沾邊的唯一一筆是居民心情好時自己哼的歌（voxel_humming）——玩家完全插不上手；集會鐘能召居民走向你，但你到了之後什麼都做不了。本模組讓玩家工作台合成一把手風琴、主動按鍵開演，附近閒著的居民會停下腳步聆聽、跟著搖擺、記進交情，世界第一次由玩家主動表演、居民當場駐足回應（自主提案切片，ROADMAP 977，接續 976 蒸汽獨輪車後再換一個全新維度）
mod voxel_prosperity; // 領地繁榮 v1·領地保護（963起）讓玩家的家別人動不得，但那塊地從立牌起就靜止不動——不長大也不產出。本模組讓「家」牌只要還在就隨真實時間持續累積繁榮度，信任名單裡的朋友造訪能加快成長，練到門檻起被動產出乙太幣（領主回家自動收取），練到更高等級起路過居民會被吸引駐足由衷讚賞——零新熱鍵、零新合成物品，正面回應 reviewer 退回 #1265 時定下的三條判準（自主提案切片，ROADMAP 979）
mod voxel_tool_wear; // 工具耐久＋鐵匠代客保養 v1·工欲善其事（790）讓鎬/斧/鏟有機率多收一份材料，但工具本身做出來就永遠堪用——全專案 39 刀自主提案切片裡唯一空白的資源經濟軸線。本模組讓真實使用（挖礦/砍樹/鋤地）磨損工具、磨到門檻後 790 加成暫時失靈，帶著壞掉的工具走近任一位居民付費保養即可修好；常找同一位居民修，她會漸漸把你當老主顧、保養費隨拜訪次數累積打折——零新熱鍵、零新合成物品、觸發頻率等同挖礦本身，直接回應 reviewer 對 976~980 定下的判準（自主提案切片，ROADMAP 981）
mod voxel_home_decor; // 家居擺設 v1·931 讓玩家能合成小地毯/花盆/小圓桌/掛旗四樣純裝飾傢俱，但「送給居民」這條路從沒真的接上——送出去材料憑空消失，甚至連名字都沒登記（fallback「物品」），跟蓋家（652/990）當年「材料憑空生方塊」是同一種缺口。本模組讓送的家具真的擺進她家、持久顯示、寫進記憶；鄰居走近另一位居民的家，看到自己家還沒有的擺設會心生嚮往、記下想要哪一款，等你送對東西才實現——讓「一戶人家的品味」第一次有機會在鄰里之間擴散（自主提案切片，換維度離開 979~981 工具/繁榮/傢俱本體，回頭補傢俱系統與居民↔居民的因果缺口）
mod voxel_colony_milestone; // 殖民地集體里程碑 v1·voxel_settle.rs 自己頭註留白「紀念柱／村莊集體里程碑／村莊地圖端點」三項留待後續刀——地圖端點已由 971 補上，紀念柱與村莊集體里程碑至今仍只認主村，殖民地無論搬進去幾位居民都沒有一刻「一起長大」的慶祝、也沒有屬於自己的村碑。本模組以聚落人口（而非主村用的命名地標數，殖民地初期地廣人稀不適用同一把尺）為指標，人口跨過 2/4/8 門檻時該座殖民地自己的居民一起慶祝、沿用既有村碑幾何在殖民地中心立起一根屬於自己的紀念柱，每座殖民地各自累計、互不相干（自主提案切片，補 voxel_settle 地基缺口，承 PLAN_ETHERVOX §7）
mod voxel_playmate; // 孩童玩伴 v1·1016 讓每個孩子會忍不住獨自玩起小把戲，但兩個孩子擦身而過時彼此毫無反應，像兩個各自跑圖的 NPC，村裡從沒有一刻讓玩家看得出「這兩個孩子是玩伴」。本模組讓兩個此刻都在閒晃、都是孩子的居民靠得夠近時會一起玩同一場遊戲、頭頂泡泡各自點名對方，童年行為第一次從「獨處」延伸成「兩人共享」——正交於血緣（sibling）與交情（clique），純粹因「同時在場＋都是孩子」自然結成（自主提案切片，接續 1016 童年新維度）
mod voxel_shipwreck; // 沉船地標 v1·河流（1005）與橋（1006）讓世界有一條值得渡的水系，木筏（1017）讓渡河多一種代步工具——但這條河至今純粹是路過的地形，既有六種地標全落在乾燥陸地或深層岩壁，沒有任何一處目的地要求玩家「先下水」才碰得到。本模組在河流固定一處深水核心埋一艘沉船殘骸，全世界唯一一處非潛水／乘筏摸不到的地標，讓水域探索第一次有了明確目的地（自主提案切片，接續 1005/1006/1017）
mod voxel_mount; // 騎乘馴養夥伴 v1·馴養羈絆線疊了餵食馴服（847/870）→跟隨（851）→取名（895）→安置召回（898）→寵愛（899）五刀，天天帶在身邊、取了名、疼過的小夥伴卻永遠只能徒步跟在腳邊——「騎乘」這個動詞至今只屬於冰冷的機械載具（蒸汽獨輪車 976／木筏 1017），有生命、記得你的寵物反而不能。本模組讓已馴服的兔／雞第一次能被騎上去，貼在你身邊精神抖擻地陪你飛奔，移動速度比純徒步快；比照 976/1017 同款「單鍵切換＋伺服器權威複驗」手法，零新持久化欄位（自主提案切片，ROADMAP 1021）
mod voxel_festival; // 村莊慶典日 v1·世界至今每一場「特別的日子」都繫在某一位居民身上（生日/婚禮/圓夢），沒發生在誰身上那天就跟平常沒兩樣；手風琴（voxel_busking）自己的頭註都明講世界從沒有「集體慶祝」，煙火/生日只是事件驅動的一次性慶典，沒有排定好、全村共同、一再循環的節日。本模組讓世界時鐘每隔固定天數迎來一個慶典日，那一整天閒著的居民更容易顯露歡慶的心情，讓世界第一次有了屬於全體、而非某一位居民的節慶節奏（自主提案切片，ROADMAP 1022，接續 1017/1019/1021，換維度離開個體事件/天氣×生物，改做全村排定行事曆）
mod voxel_wear; // 染色頭巾 v1·世界近 200 刀以來，居民有詳盡的家/建物/稱號當視覺身分，玩家與居民的方塊小人卻從出生到終老連一絲外觀都沒變過——命名/稱謂/名牌全是文字層，沒有任何系統動過「看起來長什麼樣」。本模組讓玩家用葉片＋野花染色合成頭巾，戴上後其他玩家立刻看得見；送給居民她會一直戴著，身分第一次不只是文字，而是看得見的外觀（自主提案切片，ROADMAP 1023，換維度離開行為/關係/天氣/地圖，改做「看起來長什麼樣」）
mod voxel_hotspring_rest; // 居民也去溫泉歇腳 v1·838/839 讓溫泉成為世界第二種可探索地標，泡進去能加速回血、緩解飢餓——但那份功能性回饋至今只服務玩家，居民對這泓暖泉視若無睹，地標系統與居民的日常至今是兩條互不相干的線。本模組讓閒著的居民偶爾放下手邊的事，走去村莊附近那泓已知的溫泉泡個舒服的澡再回家；若恰好巧遇你也在泡，會多說一句、記進心裡——地標第一次不只是玩家的祕境，也成了居民會去、也會記得的地方（自主提案切片，接續 838/839，換維度：把「地標」與「居民自主行為」第一次接在一起，ROADMAP 1025）
mod pet;
mod pet_fetch;
...
async fn main() {
    ...
    let app = Router::new()
        ...
        // 乙太方界·邀居同住 v1（ROADMAP 972）：目前登入帳號正在同住的居民 id 清單，僅認 cookie。
        ...
        // 乙太方界·引夢使者印記（自主提案切片）：你在風禾方界為居民做過的具體善舉彙總 JSON（唯讀）。
        .route("/voxel/envoy_marks", get(voxel_ws::voxel_envoy_marks_handler))
        // 乙太方界·殖民地真居住（架構級大弧 v1）：野外殖民地名冊＋現居居民彙總 JSON（唯讀）。
        .route("/voxel/colonies", get(voxel_ws::voxel_colonies_handler))
        // 乙太方界·眾力共築·乙太燈塔（自主提案切片）：全服募材進度＋貢獻榜 JSON（唯讀）。
        .route("/voxel/lighthouse", get(voxel_ws::voxel_lighthouse_handler))
