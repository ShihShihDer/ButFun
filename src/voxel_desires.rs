...
/// `sparked_by` 的見賢思齊哨兵值（`voxel_envy`，自主提案切片 858）：居民路過一座
/// 已命名地標、親眼所見而心生嚮往的心願用這個標記（非玩家、非禱告、非好奇心），
/// 供顯示/思考 prompt 判斷來源、且蓋家完工的「指名感謝玩家」邏輯知道這不是一位真人玩家。
pub const ENVY_SPARK: &str = "見賢思齊";
