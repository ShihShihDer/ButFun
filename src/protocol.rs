...
pub enum ClientMsg {
    ...
    Cheer,
    /// 同伴扶起（ROADMAP 464）：玩家按「🤝 扶起」把附近倒下的旅人扶起來。伺服器以扶人者自己的
    /// 權威座標，挑出 `companion_revive::REVIVE_RADIUS_PX` 內最近的一位倒地者就地扶起（半血起身、
    /// 不再被傳回新手村）。無 payload——對象一律用扶人者自己的權威座標挑（防隔空救人）。
    /// 自己倒地中 / 在室內 / 附近沒人倒地 / 超量靜默忽略。
    HelpUp,
    /// 流星雨共願（ROADMAP 471）：流星雨期間玩家按「✨ 許願」，向劃過的星河許下一個心願。
    /// 無 payload——只是一次全服共享的療癒儀式：伺服器把這名玩家記進本場許願者集合（每場每人
    /// 至多算一次、冪等），全服廣播一句「某某向流星雨許了願」＋累計共願人數；流星雨落幕時再由
    ...
    /// 搆不著 / 是自己的 / 已讚過 / 雪人已融化一律靜默忽略。零經濟、零持久化、純記憶體、重啟清零。
    CheerSnowman {
        id: u32,
    },
    ...
    /// 訪客／無地者／離田太遠則回一句「靠近農地才能澆」的溫和提示。
    WaterAll,
    /// 一鍵收成（ROADMAP 446）：玩家站在自己農地旁按「✨ 一鍵收成」，把整塊田所有已成熟的
    /// 作物一次收完，省去逐格點擊（對稱於 422 一鍵澆水）。不帶座標——伺服器一律用玩家自己的
    /// 權威座標判定是否在農地可及範圍內（防隔空收成）。在範圍內才收、收完單播回報收了幾株、
    /// 拿了多少乙太；訪客／無地者／離田太遠則回一句「靠近農地才能收成」的溫和提示。
    /// 與逐格手收完全等價（同樣的品質／沃土結算），只是一次做完，零新經濟。
    HarvestAll,
    ...
    /// 不滿足條件則靜默忽略。
    BeginGuard,
    /// 臨陣格擋：按下格擋（ROADMAP 408）。備防進行中，玩家瞄準甜蜜點送出。
    ...
    Dodge,
    /// 起蓄一記重擊（ROADMAP 423 蓄力重擊）。玩家按住攻擊鈕開始蓄力。
    /// 伺服器驗格（未倒地＋冷卻過）後開一趟蓄力，蓄力進度隨快照廣播渲染蓄力環；
    /// 已在蓄力則重新起蓄。不滿足條件則靜默忽略。蓄力本身無副作用，重擊在放開時兌現。
    BeginCharge,
    /// 放開蓄力（ROADMAP 423）。玩家放開攻擊鈕，伺服器依蓄力時間結算檔位：
    /// 蓄足半蓄以上即備一記「待擊」重擊（限時存活、被下一次攻擊消費），並起冷卻。
    /// 沒在蓄力（或蓄不足門檻）則只是輕揮、不備重擊。前端放開後緊接著送 `Attack` 兌現。
    ReleaseCharge,
    ...
    pub struct CampfireView {
        ...
        /// 圍爐人數（眾人拾柴 ROADMAP 545）——前端據此把火畫得更旺。舊前端忽略此欄、向後相容。
        pub gather_count: u8,
        /// 有效暖意半徑（像素，眾人拾柴 545）——已含圍爐人數加成；前端用它畫暖意圈、
        /// 後端用同值判定敵人安撫，兩端同口徑。舊前端讀不到時退回固定半徑、向後相容。
    ...
    pub struct SnowmanView {
        ...
        pub style: u8,
        /// 累積愛心數（ROADMAP 479 雪人讚賞）——附近玩家走近按讚累加，前端在雪人旁顯示「♥ N」。
        /// 舊客戶端缺此欄會以 0 解析（#[serde(default)]），向後相容。
        #[serde(default)]
        pub cheers: u16,
    }
...
pub enum ServerMsg {
    ...
    /// `from_x/from_y`：攻擊者世界座標；`to_x/to_y`：命中目標座標（miss 時為 0）；`hit`：是否命中。
    /// ROADMAP 510：加入 to_x/to_y 供前端繪製飛矢軌跡（#[serde(default)] 確保舊客戶端相容）。
    RangedHit { from_x: f32, from_y: f32, #[serde(default)] to_x: f32, #[serde(default)] to_y: f32, hit: bool },
    ...
    /// 圍爐分食（ROADMAP 462）：有人在旅人身旁吃下一道暖食料理，香氣分了「半份暖意」給
    /// 附近 `recipients` 位旅人。全服廣播，前端在 `(x, y)` 飄一陣暖食香氣特效；受惠者頭頂的
    /// 暖食光暈本就隨快照的 `well_fed` 同步亮起，故此事件只負責「分食的那一瞬」氛圍。
    /// 一次性事件、不入快照、不持久化。`eater`＝吃飯者顯示名、`recipients`＝惠及人數（≥1 才廣播）。
    MealShared {
        eater: String,
        x: f32,
        y: f32,
    ...
    /// 同伴扶起（ROADMAP 464）：`rescuer` 把倒下的 `target` 扶了起來（就地半血起身、不再被傳回
    /// 新手村）。全服廣播，前端在 `(x, y)`（被救者起身處）迸一道暖光救援特效；被救者頭頂的血條
    /// 本就隨快照血量同步回滿一半，故此事件只負責「被扶起的那一瞬」氛圍與當事人的播報。
    /// 一次性事件、不入快照、不持久化。`rescuer`/`target` 為雙方顯示名，供兩位當事人前端播報。
    PlayerRevived {
        rescuer_id: Uuid,
        rescuer: String,
        target_id: Uuid,
    ...
    /// 流星雨共願（ROADMAP 471）：有人向當前這場流星雨許下了心願。全服廣播——許願者本人前端
    /// 報讀「你向流星雨許了願」並把 HUD 標為已許願，旁人只默默更新「目前共願人數」。`player_id`/
    /// `name` = 許願者；`total` = 本場累計的不重複許願人數。一次性事件、不入快照、不持久化。
    WishMade {
        player_id: Uuid,
        name: String,
        total: u32,
    ...
    /// 草原細雨庇護（ROADMAP 496）：細雨中戶外玩家回了血。直接送給被回血的玩家（透過
    /// whisper_senders 私訊），前端在頭頂顯示「🌧️ +N」飄字，讓玩家感受「天時」的庇護。
    /// 一次性事件、不入快照、不持久化、重啟清零。
    RainHeal {
        /// 本次實際回復的 HP 量（通常為 1，累積器驅動；>1 在 dt 異常大時才發生）。
        amount: u32,
    },
    ...
    /// 前端據此顯示可消除的「歡迎回來」卡片。
    ReturnSummary {
        /// 個人農田地塊裡成熟待收的作物數（伺服器重啟後歸零）。
        ripe_crops: u32,
        /// 個人牧場地塊裡待領取的蛋數（伺服器重啟後歸零）。
        eggs_ready: u32,
        /// 今日任務已完成條數（尚無記錄時為 0）。
        daily_quests_done: u32,
        /// 今日任務總條數（尚無記錄時為 0）。
    ...
    /// 雪人被讚賞（ROADMAP 479）。單播——同時送給讚賞者（確認「你給 XX 的雪人按了讚」）與
    /// 堆雪者（暖心通知「XX 讚賞了你的雪人，共 N 個愛心」）。前端比對 `builder_name` 與自己暱稱
    /// 決定顯示哪句；`cheers` = 該雪人最新累積愛心數。一次性事件、不入快照、不持久化、零 migration。
    SnowmanCheered {
        /// 被讚賞的雪人 id（前端可在該雪人旁迸一顆愛心、即時把愛心數刷成 `cheers`）。
        id: u32,
        /// 該雪人最新累積愛心數。
        cheers: u16,
        /// 讚賞者暱稱。
        by_name: String,
        /// 堆雪者暱稱（前端比對自己 → 顯示「你的雪人被讚賞」道賀）。
        builder_name: String,
    },
    /// 人氣聚會湧現（ROADMAP 342）：一名高人氣玩家（341 攢到「受歡迎」以上）身邊聚起足夠人潮，
    /// 伺服器判定一場「聚會」繞著他成形。全服廣播、前端在該玩家**腳下**亮起一圈發光的人氣聚會圈、
    /// 並在世界頻道宣告「在 XX 周圍聚起了一場熱鬧的聚會」。`host_id`/`host_name` ＝ 聚會主人；
    /// `guests` ＝ 成局當下圍在他身邊的賓客數。一次性事件、不入快照、不持久化、零 migration；
    /// 聚會圈隨主人移動（前端依其 id 的即時座標畫，主人走到哪圈跟到哪）。
    PopGatheringStarted {
        host_id: Uuid,
        host_name: String,
        guests: u32,
    },
    /// 人氣聚會落幕（ROADMAP 342）：主人身邊人潮散去（經散場緩衝確認）、聚會結束。
    /// 前端據此淡出該主人腳下的聚會圈。`host_id` ＝ 原聚會主人。一次性事件、不入快照、不持久化。
    PopGatheringEnded {
        host_id: Uuid,
    },
    ...
    LocaleEntered {
        ...
        subtitle: String,
        initial: bool,
        /// 遠遊見聞（ROADMAP 411）：這次踏入是否為「本趟初次踏足這處」。
        /// true ＝地名卡綴「✨初次踏足」、響發現清音、報讀；false ＝舊地重遊（純地名卡）。
        /// `#[serde(default)]` 向後相容（舊前端忽略即可）。
        #[serde(default)]
        first_footfall: bool,
        /// 本趟至此踏足過的不同地方數（足跡計數，僅 `first_footfall` 時有意義）。
        #[serde(default)]
        tally: u32,
        /// 這次初次踏足攢的探索者熟練度 XP（可能為 0：本趟 XP 已封頂，仍是真初次踏足）。
        #[serde(default)]
        xp_reward: u32,
    },
    ...
    AttackHit {
        ...
        is_kill: bool,
        is_crit: bool,
        /// 蓄力重擊命中（ROADMAP 423）：1=半蓄、2=滿蓄、0=普通（含暴擊）。
    ...
    /// 正當時收割飄字（ROADMAP 560）：玩家在「黃金熟成期」採收作物時私信本人，
    /// 前端在收割位置飄出「✨ 正當時！＋N」慶祝、播報無障礙提示。
    /// 一次性事件、不入快照、不持久化——僅私信，零頻寬增量。
    FarmHarvest {
        /// 收割位置（世界座標，取玩家當下位置）。
        x: f32,
...
pub struct PlayerView {
    ...
    pub exp: u32,
    /// 生態圖鑑已發現物種 bitmask（ROADMAP 333）。前端只取「自己那位」的來畫圖鑑面板，
    /// 並與上一幀比對、新點亮的位元噴「新發現」飄字。
    pub codex: u64,
    ...
    #[serde(default, skip_serializing_if = "is_zero_f32")]
    pub pet_y: f32,
    /// 寵物此刻是否正在跟別的寵物玩耍（ROADMAP 344「寵物玩伴嬉戲」）。前端據此在湊起來玩的
    ...
    /// 前端用同一條公式（`guard::beat_fraction`）渲染玩家頭頂收束的格擋環，
    /// 自己與旁觀者都看得到（旁觀者看別人正看準時機格擋）。
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub guard_secs: Option<f32>,
    /// 此刻乙太護盾的卸傷強度（0~85）；沒上盾＝None（略過序列化）。
    ...
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub charge_progress: Option<f32>,

    // ── 遠遊見聞（ROADMAP 411）────────────────────────────────────────────────
    /// 本趟遠遊踏足過的不同地方數；0＝還沒踏足記錄（略過序列化）。
    /// 前端 HUD 畫「🧭 遠遊見聞 N 處」足跡計數，隨自身快照廣播。
    #[serde(default, skip_serializing_if = "is_zero_u32")]
    pub wayfare_count: u32,
