...
// ─── ROADMAP 312：春日嗅花 ───────────────────────────────────────────────────
/// 春日嗅花自發起意機率（每幀）——偏低（與 307 喘氣／308 哆嗦 PROB 0.03 同量級，多數幀不觸發）。
const NUZZLE_PROB: f32 = 0.03;
/// 嗅花單段最短時長（秒）——低頭湊近初綻的春花輕嗅一陣再抬頭續行（比呵氣久、與喘氣／哆嗦同量級）。
const NUZZLE_DURATION_MIN: f32 = 2.0;
/// 嗅花單段最長時長（秒）。
const NUZZLE_DURATION_MAX: f32 = 4.0;

// ─── ROADMAP 315：秋日囤食貼膘 ───────────────────────────────────────────────
/// 秋日囤食自發起意機率（每幀）——偏低（與 307 喘氣／308 哆嗦／312 嗅花 PROB 0.03 同量級，多數幀不觸發）。
const FATTEN_PROB: f32 = 0.03;
/// 囤食單段最短時長（秒）——低頭加緊啃食一陣再抬頭續行（比嗅花久些，讀起來像埋頭進食、為過冬增膘）。
const FATTEN_DURATION_MIN: f32 = 2.5;
/// 囤食單段最長時長（秒）。
...
// ─── ROADMAP 306：黃昏暮鳴（dusk chorus）────────────────────────────────────────
// 305 拂曉鳥鳴大合唱把「晝鳥鳴接時辰」的第一筆落在「破曉」——晨光初上、滿林齊鳴。可一日的鳥鳴
// 有頭也有尾：天剛破曉的「dawn chorus」之外，現實裡野鳥在日落前同樣有一陣「evening chorus」（暮鳴）
// ——夕陽西沉、入夜歸巢（210 夜眠）之前，鳥群再放聲鳴唱一陣作為一日的告別曲。305 只演了「破曉」這
// 一頭，正午照常疏鳴、黃昏卻與正午鳴得一樣疏。本切片把 305 的同一手法（is_dawn 走欄位）搬到黃昏：
// 黃昏時段（後端 is_dusk，由 game.rs 以權威日夜時鐘的 Phase::Dusk 判定、走欄位餵入，與 305 is_dawn、
// 302 moon_full 同慣例），野鳥自發起鳴的機率拉高、一段鳴唱拉長——但刻意都介於「平常」與「破曉」之間：
// 黃昏的暮鳴比正午密、卻不如破曉那般滿林齊鳴，是入夜前一陣漸漸收歇的告別曲。與 305 拂曉合唱對成
// 「破曉／黃昏」一對，把「晝鳥鳴接時辰」從只演破曉這一頭，補成「晨起齊鳴—正午疏鳴—暮色再鳴—入夜
// 歸巢」一日聲景的完整循環（305 dawn chorus → 221 白晝鳴 → 306 dusk chorus → 210 夜眠）。221 鳴聲呼應
// 照舊（仍走既有 Chirping 狀態與 chirping_snap、CHIRP_JOIN_PROB，無新狀態、無新協議欄位）；前端畫頭頂
// 符號時自讀廣播的 daynight.phase，破曉／黃昏這兩個合唱時辰皆畫一串更密的 🎶、正午畫 🎵，一眼看得出
// 「晨昏的合唱與正午不同」。純啟發式、零 LLM、零 tick 簽名改動、零協議改動、零新狀態、零持久化、
// 零 migration、記憶體模式。
/// 黃昏時段野鳥自發起鳴的機率——高於平常（CHIRP_PROB 0.015）、卻刻意低於破曉（DAWN_CHIRP_PROB 0.05），
/// 表現「暮色裡的鳴唱比正午密、卻不如破曉那般滿林齊鳴」，是入夜前一陣漸收的告別曲。
const DUSK_CHIRP_PROB: f32 = 0.035;
/// 黃昏一段鳴唱的最短／最長時長（秒）——介於平常（1.5~3.0）與破曉（2.5~4.5）之間，暮鳴比正午長、
/// 卻不如晨鳴拖得那般久。
const DUSK_CHIRP_DURATION_MIN: f32 = 2.0;
const DUSK_CHIRP_DURATION_MAX: f32 = 3.5;
...
// 標完就走，旁邊另一隻同類視若無睹，留味只是個自說自話的單向動作。本切片把這條線補成真正的雙向社交：
// 白天平靜、無獵可追的掠食者，若「聞到」附近 SNIFF_SEE_RADIUS 內有同類正留下一記新鮮的領地記號
//（marking_snap：本幀正在 Marking 的掠食者座標快照），便偶爾以 SNIFF_PROB 停下、抬頭一嗅一嗅地讀這記
// 界味、辨認「這是誰的地盤」（新增 Sniffing 狀態、頭頂浮一枚隨一嗅一頓上下輕動的 👃）。讓 293 留下的
// 記號第一次被另一個個體真正讀到、回應——直接回應 reviewer 一路要的「讓不同個體真的彼此牽動」。
/// 一隻掠食者「聞到」同類新鮮領地記號的感知半徑——略大於 287 碰鼻問安 GREET_RADIUS：氣味比碰鼻傳得遠
/// 一些（老遠就嗅得到界味），但仍是貼近的鄰里範圍（不是全圖都聞得到）。
const SNIFF_SEE_RADIUS: f32 = 220.0;
/// 聞到同類新鮮記號後本幀停下讀味的機率——與 293 嗅標（MARK_PROB 0.03）同量級偏低：讀味是偶爾的回應，
/// 多數時候即便附近有同類在標、這隻仍照常巡遊（不是一聞到就必停）。
const SNIFF_PROB: f32 = 0.045;
/// 一次讀味的最短／最長時長（秒）——湊上去一嗅一嗅辨認界味，與 293 嗅標（1.5~3.0s）同量級的短促停頓。
/// 期間威脅一旦逼近一律優先停下奔逃（讀味永遠讓位逃命，由上游 prey_snap／flee 守衛把關）。
...
// ─── ROADMAP 297：雨後抖水（rain shake-off／雨停後抖落皮毛羽毛上的水珠）──────────────
// 296 雨中避雨（🌧️）把「動物 × 天氣」這條維度由「下雨時縮身避雨」起了頭——可那筆只演到「雨還在下」。
// 雨總會停，而雨一停、最招牌的一幕正是被淋濕的動物渾身一抖、把皮毛羽毛上的水珠甩開（狗淋雨後的甩身、
// 鳥抖鬆羽毛抖落雨水）。本切片給 296 補上「雨後」這後半段，與 296 雨中避雨對成「雨中／雨後」一對：
// 雨剛停的一小段時間窗（POST_RAIN_WINDOW）內，白天平靜歇息／漫步的草食獸（鹿／鳥／小獸）偶爾停下、
// 原地用力一抖把身上的雨水甩開（新增 Shaking 狀態、頭頂浮一枚被甩開、四下飛濺的 💦），抖一小陣就鬆下來
// 回復閒晃。**雨停的瞬間由 set_raining 偵測（raining→not raining 的下降緣）開一段時間窗、tick 逐幀倒數**，
// 故抖水只在「剛淋過雨」的當口浮現——雨停得久了（窗口耗盡）就不再抖，晴了一整天的乾獸自然不會無端抖水。
// 逐幀低機率觸發 → 雨一停，剛才縮著避雨的獸群由近而遠錯落地一隻隻抖開水珠（仿 296 避雨的漣漪式起頭），
// 而非同幀整群瞬間齊抖。**與 296 雨中避雨（🌧️）對成「雨中／雨後」一對、以「正在下雨／雨剛停」乾淨區隔**：
// 避雨要 is_raining、抖水要 !is_raining 且在雨停窗口內，兩者畫面上永不同時出現。**晝起意**（夜間草食獸
// 另走夜眠分支，與 296 一致）。**威脅永遠優先**：掠食者／玩家一旦逼進 FLEE_RADIUS 一律改全速奔逃（抖水
// 永遠讓位逃命）。**只屬於草食獸**（Phase 4 本就只處理獵物，故天然只草食獸觸發）。純啟發式、零 LLM、可測、
// 零 tick 簽名改動（post_rain 走新增的 manager 欄位、由 set_raining 的下降緣設窗、tick 內逐幀倒數）、
// 零協議改動（新增的 shaking 字串沿用 state_str；計時隨狀態變體攜帶，無新欄位）、零持久化、零 migration、記憶體模式。
/// 雨停後仍會觸發抖水的時間窗（秒）——雨一停就開窗，逐幀倒數歸零即關。讓抖水只在「剛淋過雨」的當口浮現，
/// 雨停久了的乾獸不再無端抖水（也給剛才縮著避雨的獸群留足由近而遠錯落抖開的餘裕）。
const POST_RAIN_WINDOW: f32 = 12.0;
/// 雨停後平靜的草食獸偶爾停下、用力一抖甩開水珠的逐幀機率——與 296 避雨 SHELTER_PROB(0.05) 同量級：
/// 偏低，讓抖水是「雨一停、一隻隻陸續抖開水珠」的漣漪式起頭，而非同幀整群瞬間齊抖。
const SHAKE_PROB: f32 = 0.05;
/// 一陣抖水的最短／最長時長（秒）——比避雨／反芻更短促的一下下用力抖動（甩水是一陣急抖、非耐著性子的久佇）。
...
enum WildlifeState {
    ...
    /// ROADMAP 294：聞味讀界——白天無獵可追的平靜掠食者（野狼／野狐，二者皆會）「聞到」附近同類正留下一記
    /// 新鮮的領地記號（marking_snap）時，偶爾湊上前停下、抬頭一嗅一嗅地讀這記界味、辨認是誰的地盤（前端畫成
    /// 原地不動、頭頂浮一枚 👃）。原地不動（不更新座標）、sniff_timer 倒數，讀完再起身回到巡遊（朝家附近的下一個
    /// 漫遊目標，與 tick_mark／tick_lick／tick_doze 同模式）。獵物一旦進入搜尋範圍，掠食者 phase 在讀味分支之前
    /// 就已改走狩獵——讀味永遠讓位狩獵。與 293 嗅標（💧，留下記號）對成「留味／讀味」的雙向社交一對：一個留、
    ...
    /// ROADMAP 297：雨後抖水——雨剛停的時間窗（POST_RAIN_WINDOW）內，白天平靜的草食獸（鹿／鳥／小獸）
    /// 偶爾停下、原地用力一抖把皮毛羽毛上的水珠甩開（前端畫成原地急抖、頭頂浮一枚四下飛濺的 💦）。
    ...
    /// ROADMAP 312：春日嗅花——哺乳草食獸（鹿／小獸）在春季白天（is_spring）平靜時，偶爾低頭湊近初綻的
    /// 春花輕嗅一陣（頭頂浮 🌸）。原地不動（不更新座標）、nuzzle_timer 倒數，到期就抬頭回到巡遊；嗅花中
    /// 若有威脅／獵物一律優先中斷改逃竄／狩獵。與 294 嗅標（Sniffing，讀同伴留在界標上的氣味訊息）截然
    ...
    /// ROADMAP 315：秋日囤食貼膘——草食走獸（鹿／小獸）在秋季白天（is_autumn）平靜時，偶爾停下、低頭加緊
    /// 啃食、為過冬儲積脂肪（頭頂浮 🍂）。原地不動（不更新座標）、fatten_timer 倒數，到期就抬頭回到巡遊；
    /// 囤食中若有威脅／獵物一律優先中斷改逃竄／狩獵。與 311 秋日群鳥集結（🧭，野鳥南遷前的集結）對成
    ///「秋天裡，鳥集結、獸貼膘」一對——同是社交／覓食物種在嚴冬來臨前的「過冬準備」，物種閘恰好互補
    ///（311＝野鳥、本切片＝草食哺乳獸），把秋季從只有野鳥補成獸鳥俱全；也與 312 春嗅花（🌸）／307 夏喘氣
...
impl Wildlife {
    ...
    /// ROADMAP 312：春日嗅花——嗅花中（Nuzzling）原地不動、nuzzle_timer 倒數；嗅夠了一陣（耗盡）就抬頭
    /// 回到巡遊（朝家附近的下一個漫遊目標，與 tick_puff 同模式）。只在 Nuzzling 狀態下生效（呼叫端已確保
    /// 此隻為哺乳草食獸、春季白天、平靜無威脅；獵物／威脅一旦出現，更前面的 phase 已改走狩獵／逃竄，不會
    /// 走到此分支——嗅花永遠讓位即時反應）。
    fn tick_nuzzle(&mut self, dt: f32, rng: &mut StdRng) {
        if let WildlifeState::Nuzzling { nuzzle_timer } = self.state {
            let remaining = nuzzle_timer - dt;
            if remaining <= 0.0 {
                let (wx, wy) = random_target(self.home_x, self.home_y, WANDER_RADIUS, rng);
                self.state = WildlifeState::Wandering { target_x: wx, target_y: wy, wander_timer: 5.0 };
            } else {
                self.state = WildlifeState::Nuzzling { nuzzle_timer: remaining };
            }
        }
    }

    /// ROADMAP 315：秋日囤食貼膘——囤食中（Fattening）原地不動、fatten_timer 倒數埋頭進食；吃夠了一陣
    /// （耗盡）就抬頭回到巡遊（朝家附近的下一個漫遊目標，與 tick_nuzzle 同模式）。只在 Fattening 狀態下
    /// 生效（呼叫端已確保此隻為草食走獸、秋季白天、平靜無威脅；獵物／威脅一旦出現，更前面的 phase 已改走
    /// 狩獵／逃竄，不會走到此分支——囤食永遠讓位即時反應）。
    fn tick_fatten(&mut self, dt: f32, rng: &mut StdRng) {
        if let WildlifeState::Fattening { fatten_timer } = self.state {
            let remaining = fatten_timer - dt;
            if remaining <= 0.0 {
                let (wx, wy) = random_target(self.home_x, self.home_y, WANDER_RADIUS, rng);
                self.state = WildlifeState::Wandering { target_x: wx, target_y: wy, wander_timer: 5.0 };
            } else {
                self.state = WildlifeState::Fattening { fatten_timer: remaining };
            }
        }
    }
    ...
    /// ROADMAP 294：聞味讀界——讀味中（Sniffing）原地不動、倒數計時；讀完（sniff_timer 耗盡）就起身回到巡遊
    /// （朝家附近的下一個漫遊目標，與 tick_mark／tick_lick／tick_doze 同模式）。只在 Sniffing 狀態下生效（呼叫端
    /// 已確保此隻為掠食者、白天、無獵可追的平靜空檔且附近有同類新鮮記號；獵物一旦出現，掠食者 phase 在更前面的
    /// 獵物搜尋就已改走狩獵，不會走到此分支——讀味永遠讓位狩獵）。
    fn tick_sniff(&mut self, dt: f32, rng: &mut StdRng) {
        if let WildlifeState::Sniffing { sniff_timer } = self.state {
            let remaining = sniff_timer - dt;
            if remaining <= 0.0 {
                let (wx, wy) = random_target(self.home_x, self.home_y, WANDER_RADIUS, rng);
                self.state = WildlifeState::Wandering { target_x: wx, target_y: wy, wander_timer: 5.0 };
            } else {
                self.state = WildlifeState::Sniffing { sniff_timer: remaining };
            }
        }
    }
    ...
    /// ROADMAP 297：雨後抖水——抖水中（Shaking）原地用力一抖甩開水珠（不更新座標，急抖定格由前端以 💦
    /// 演繹），shake_timer 倒數。抖夠了一陣就鬆下來、挑家附近的下一個漫遊目標回巡遊。只在 Shaking 狀態下
    /// 生效（呼叫端已確保此隻為草食獵物、白天、雨剛停的時間窗內、且無威脅逼進 FLEE_RADIUS——那種情形更前面
    /// 就已改走逃竄，抖水永遠讓位逃命）。與 tick_shelter 同模式，但抖水不繫於 is_raining（雨停才抖、只看計時）。
    fn tick_shake(&mut self, dt: f32, rng: &mut StdRng) {
        if let WildlifeState::Shaking { shake_timer } = self.state {
            let remaining = shake_timer - dt;
            if remaining <= 0.0 {
                let (wx, wy) = random_target(self.home_x, self.home_y, WANDER_RADIUS, rng);
                self.state = WildlifeState::Wandering { target_x: wx, target_y: wy, wander_timer: 5.0 };
                return;
            }
            self.state = WildlifeState::Shaking { shake_timer: remaining };
        }
    }
...
pub struct WildlifeManager {
    ...
    raining: bool,
    /// ROADMAP 297：雨停後仍會觸發抖水的剩餘時間窗（秒）——set_raining 在偵測到雨停（raining→not raining
    ...
    is_dawn: bool,
    /// ROADMAP 306：本幀是否為黃昏時段（後端權威時辰）——game.rs 每幀於 tick 前以權威日夜時鐘的
    /// `Phase::Dusk` 判定、經 `set_dusk` 餵入（沿用 305 is_dawn 的「走欄位、零 tick 簽名改動」慣例）。
    ...
    is_autumn: bool,
    /// ROADMAP 312：本幀是否為春季——game.rs 每幀於 tick 前以權威 `app.season` 判定後呼叫。供草食獸
    /// 「春日嗅花（低頭湊近初綻春花輕嗅）」判定（走欄位、不動 tick 簽名，沿用 307/308/311 季節旗標同慣例）。
    /// 預設 false（非春季、不嗅花），故既有測試無須改動、行為與本切片前逐位元一致。
    is_spring: bool,
}
...
impl WildlifeManager {
    ...
    /// ROADMAP 297：兼偵測「雨停的下降緣」（上幀在下雨、本幀不下雨）——一停就開一段 POST_RAIN_WINDOW
    /// 的抖水時間窗（tick 內逐幀倒數），讓草食獸只在「剛淋過雨」的當口偶爾抖水甩水。
    pub fn set_raining(&mut self, raining: bool) {
        if self.raining && !raining {
            // 雨剛停：開一段抖水時間窗（剛才縮著避雨／淋著雨的獸群，這段時間內偶爾起身抖落水珠）。
    ...
    /// ROADMAP 306：更新本幀時辰（是否為黃昏）——game.rs 每幀於 `tick` 前以權威日夜時鐘的
    /// `Phase::Dusk` 判定後呼叫，餵入後端權威時辰。供黃昏時段野鳥「暮鳴」判定（自發起鳴機率拉高、
    ...
    /// ROADMAP 312：更新本幀季節是否為春季——game.rs 每幀於 `tick` 前以權威 `app.season` 判定後呼叫。
    /// 供草食獸「春日嗅花（低頭湊近初綻春花輕嗅）」判定（走欄位、不動 tick 簽名）。
    ...
    pub fn tick(
        ...
        // ROADMAP 312：本幀是否春季——供草食獸「春日嗅花（低頭湊近初綻春花輕嗅）」判定（game.rs 已於 tick 前更新）。
        let is_spring = self.is_spring;
        ...
        // ROADMAP 294：聞味讀界——本幀「正留下一記新鮮領地記號的掠食者」座標快照（野狼／野狐，Marking 狀態）。
        // 供下方另一隻平靜掠食者「聞到」附近 SNIFF_SEE_RADIUS 內這記新鮮界味、湊上前停下讀味（Sniffing）。
        // 只認當幀正在 Marking 的個體：讀的是「同類剛留下／正留下的新鮮記號」（標記者一起身離開 Marking，此快照
        // 即不含牠）。讀味者自身在 Resting/Wandering（非 Marking），不會把自己算進來，故無需另行排除自己。
        let marking_snap: Vec<(f32, f32)> = self.animals.iter()
            .filter(|a| a.alive && a.kind.trophic_level() == TrophicLevel::Predator
                && matches!(a.state, WildlifeState::Marking { .. }))
            .map(|a| (a.x, a.y))
            .collect();
                    ...
                    if let Some(target_kind) = pred_kind.hunts() {
                        ...
                        if let Some(&(target_id, _, npx, npy)) = nearest {
                                ...
                                }
                            {
                                ...
                                // 的平靜空檔延續、永遠讓位給狩獵。
                                a.tick_mark(dt, rng);
                            } else if matches!(a.state, WildlifeState::Sniffing { .. }) {
                                // ROADMAP 294：已在聞味讀界中——把這一段讀味走完（原地不動、計時倒數，讀完
                                ...
                                && rng.gen::<f32>() < MARK_PROB
                            {
                                ...
                                && matches!(a.state, WildlifeState::Resting { .. } | WildlifeState::Wandering { .. })
                                && nearest_in_range(a.x, a.y, &marking_snap, SNIFF_SEE_RADIUS).is_some()
                                && rng.gen::<f32>() < SNIFF_PROB
                            {
                                // ROADMAP 294：白天無獵可追的平靜掠食者——「聞到」附近有同類正留下一記新鮮的
                                // 領地記號（marking_snap 在 SNIFF_SEE_RADIUS 內）時，偶爾湊上前停下、抬頭一嗅一嗅地
                                // 讀這記界味、辨認是誰的地盤（頭頂浮一枚 👃）。把 293 嗅標（💧，留下記號）那筆原本
                                // 自說自話的單向留味補成真正的雙向社交——留下的記號第一次被另一個個體讀到、回應，
                                // 與 293 對成「留味／讀味」一對。野狼／野狐二者皆會（與 285 蜷睡／291 舔毛／293 嗅標
                                // 同為兩種犬科共有的白晝閒置底色）。與 225 嗅蹤（👃，野狼循一條舊味跡低頭緩步走）
                                // 刻意區隔：讀界是「原地讀同類剛留下的新鮮記號」的一次回應、非循舊跡走，且二者皆會。
                                // 排在嗅標之後、蜷睡之前：物種專屬活動／問安／舔毛／自身嗅標優先，沒起意去做這些、
                                // 卻聞到同類新鮮記號時才湊去讀一讀界，再沒有才退回蜷睡補眠。需附近真有同類在標才起意
                                //（無記號可讀則跳過）。夜間改走夜嚎分支。獵物一旦進入搜尋範圍，更前面已改走狩獵——
                                // 讀味永遠讓位狩獵。
        ...
        for i in 0..self.animals.len() {
            ...
            if calm_at_night {
                ...
                if matches!(a.state, WildlifeState::Watching { .. }) {
                    ...
                    // ROADMAP 297：已在雨後抖水——威脅一旦真逼進 FLEE_RADIUS 就立刻中斷改全速奔逃（抖水永遠
                    // 讓位逃命），否則原地用力一抖把水珠甩開、計時倒數，抖夠了一陣就鬆下來起身回巡遊。與 296
                    // 避雨同模式：一段平靜時的定格小動作，遇險即讓位逃命。抖水不繫於天氣（雨停才抖、只看計時）。
                    if let Some((tx, ty)) = nearest_in_range(a.x, a.y, &threats, FLEE_RADIUS) {
                        a.flee_from(tx, ty);
                    } else {
                        a.tick_shake(dt, rng);
                ...
                } else if matches!(a.state, WildlifeState::Nuzzling { .. }) {
                    // ROADMAP 312：已在春日嗅花——威脅一旦真逼進 FLEE_RADIUS 就立刻中斷改全速奔逃（嗅花永遠
                    // 讓位逃命），否則原地低頭湊近春花輕嗅、計時倒數，嗅夠了就抬頭起身回巡遊。與 309 呵氣同模式：
                    // 一段平靜時的定格小動作，遇險即讓位逃命。嗅花不繫於即時光照（起意時已逢春季、嗅完這一段
                    // 只看計時，與呵氣／喘氣同）。
                    if let Some((tx, ty)) = nearest_in_range(a.x, a.y, &threats, FLEE_RADIUS) {
                        a.flee_from(tx, ty);
                    } else {
                        a.tick_nuzzle(dt, rng);
                    }
                } else if matches!(a.state, WildlifeState::Fattening { .. }) {
                    // ROADMAP 315：已在秋日囤食貼膘——威脅一旦真逼進 FLEE_RADIUS 就立刻中斷改全速奔逃（囤食
                    // 永遠讓位逃命），否則原地低頭加緊啃食、計時倒數，吃夠了就抬頭起身回巡遊。與 312 嗅花同模式：
                    // 一段平靜時的定格小動作，遇險即讓位逃命。囤食不繫於即時光照（起意時已逢秋季、吃完這一段只看
                    // 計時，與嗅花／喘氣同）。
                    if let Some((tx, ty)) = nearest_in_range(a.x, a.y, &threats, FLEE_RADIUS) {
                        a.flee_from(tx, ty);
                    } else {
                    ...
                    && rng.gen::<f32>() < SHELTER_PROB
                {
                    ...
                    && rng.gen::<f32>() < SHAKE_PROB
                {
                    // ROADMAP 297：雨後抖水——白天（!is_night）雨剛停（!is_raining 且仍在 post_rain 時間窗內）時，
                    // 平靜歇息／漫步的草食獸（鹿／鳥／小獸；Phase 4 本就只處理獵物，故天然只草食獸觸發）偶爾停下、
                    // 原地用力一抖把皮毛羽毛上的水珠甩開（轉入 Shaking 💦）。逐幀低機率觸發 → 雨一停，剛才縮著避雨的
                    // 獸群由近而遠錯落地一隻隻抖開水珠，而非同幀整群瞬間齊抖。**與 296 雨中避雨對成「雨中／雨後」一對、
                    // 以「正在下雨／雨剛停」乾淨區隔**：避雨要 is_raining（落在上面那支），抖水要 !is_raining 且在雨停
                    // 窗口內，兩者畫面上永不同時出現——把「動物 × 天氣」這條維度從「下雨時縮身避雨」演到「雨停後抖落水珠」。
                    // **排在 296 避雨之後**（正在下雨時先避雨、雨停了才抖水，順序天然不爭）；沿用 296 一系「恐懼分支之後、
                    // 白晝玩樂之前」的位置。**威脅永遠優先**：威脅一旦真逼進 FLEE_RADIUS，上面 Shaking 續算分支即改全速
                    ...
                    && rng.gen::<f32>() < PUFF_PROB
                {
                    ...
                    && !threat_near
                    && matches!(a.state, WildlifeState::Resting { .. } | WildlifeState::Wandering { .. })
                    && rng.gen::<f32>() < NUZZLE_PROB
                {
                    // ROADMAP 312：春日嗅花——春季的白天（!is_night、is_spring），平靜歇息／漫步的草食走獸（鹿／
                    // 小獸；is_mammal 守衛把野鳥排除——低頭湊近花朵輕嗅是草食獸最招牌的「春天」畫面）偶爾停下、
                    // 湊近初綻的春花輕嗅一陣（轉入 Nuzzling 🌸）。逐幀低機率觸發 → 百花盛開的草原上，獸群由近而遠
                    // 錯落地一隻隻低頭嗅花，而非同幀整群瞬間齊嗅。**與 307 夏喘氣（👅）／308 冬哆嗦（🥶）／311 秋
                    // 集結（🧭）並列「四季各有一筆」**：夏散熱、冬禦寒、秋集結、春嗅花，把「動物 × 季節」這條維度
                    // 從只演夏冬秋三季、補上獨缺的春季這一筆——season 旗標四季到齊（307 is_summer／308 is_winter／
                    // 311 is_autumn／312 is_spring）。**與 294 嗅標（Sniffing，讀同伴留在界標上的氣味訊息）截然不同**：
                    // 那是「讀資訊」的覓食前奏、這是純粹「享受春花綻放」的季節閒情，故狀態名／符號皆刻意區隔
                    //（Sniffing 無符號／Nuzzling 🌸）。**守衛**：非春季（is_spring 為偽）一律跳過。**只屬於草食走獸**
                    //（掠食的狼／狐不嗅花——嗅花是草食獸的閒情，與牠們無關，故此切片只在草食 phase 落筆、掠食迴圈
                    // 不覆蓋；有測試覆蓋鳥不嗅）。**排在 296～299 雨天反應、251/284/295 恐懼分支、307 喘氣、308 哆嗦、
                    // 309 呵氣之後、下方吃草／搔癢／嬉戲等白晝玩樂之前**：看得見的危險與雨天反應永遠優先，嗅花這樁
                    // 春日閒情則凌駕悠閒玩樂。**威脅永遠優先**：威脅一旦真逼進 FLEE_RADIUS，上面 Nuzzling 續算分支即改
                    // 全速奔逃（嗅花永遠讓位逃命）。機率先擲，多數幀一擲不中即略過。
                    let timer = rng.gen_range(NUZZLE_DURATION_MIN..=NUZZLE_DURATION_MAX);
                    a.state = WildlifeState::Nuzzling { nuzzle_timer: timer };
                } else if is_mammal
                    && !is_night
                    && is_autumn
                    && !threat_near
                    && matches!(a.state, WildlifeState::Resting { .. } | WildlifeState::Wandering { .. })
                    && rng.gen::<f32>() < FATTEN_PROB
                {
                    // ROADMAP 315：秋日囤食貼膘——秋季的白天（!is_night、is_autumn），平靜歇息／漫步的草食走獸
                    //（鹿／小獸；is_mammal 守衛把野鳥排除——埋頭增膘過冬是草食獸最招牌的「秋天」畫面）偶爾停下、
                    // 低頭加緊啃食、為過冬儲積脂肪（轉入 Fattening 🍂）。逐幀低機率觸發 → 秋風裡，獸群由近而遠
                    // 錯落地一隻隻低頭埋進草裡猛吃，而非同幀整群瞬間齊吃。**與 311 秋日群鳥集結（🧭，野鳥南遷前的
                    // 集結）對成「秋天裡，鳥集結、獸貼膘」一對**：同是物種在嚴冬來臨前的「過冬準備」，物種閘恰好
                    // 互補（311＝is_bird、本切片＝is_mammal，永不相疊），把秋季從只有野鳥補成獸鳥俱全；也與 307 夏
                    // 喘氣（👅）／308 冬哆嗦（🥶）／312 春嗅花（🌸）並列哺乳獸「四季各有一筆」（夏散熱／冬禦寒／
                    // 春嗅花／秋貼膘）。**與 312 嗅花（純享受春花綻放的閒情）截然不同**：那是「賞春」、這是過冬前
                    // 埋頭增膘的進食姿態，故狀態名／符號皆刻意區隔（Nuzzling 🌸／Fattening 🍂）。**秋季守衛**：
                    // 非秋季（is_autumn 為偽）一律跳過。**只屬於草食走獸**（掠食的狼／狐不貼膘——增膘過冬是草食獸的
                    // 事，與牠們無關，故此切片只在草食 phase 落筆、掠食迴圈不覆蓋；有測試覆蓋掠食者不貼膘）。**排在
                    // 296～299 雨天反應、251/284/295 恐懼分支、307 喘氣、308 哆嗦、309 呵氣、312 嗅花之後、下方吃草／
                    // 搔癢／嬉戲等白晝玩樂之前**：看得見的危險與雨天反應永遠優先，囤食這樁過冬本能則凌駕悠閒玩樂。
                    // **威脅永遠優先**：威脅一旦真逼進 FLEE_RADIUS，上面 Fattening 續算分支即改全速奔逃（囤食永遠
                    // 讓位逃命）。機率先擲，多數幀一擲不中即略過。
                    ...
                    }
                {
                    ...
                    && {
                        ...
                        // ROADMAP 306：黃昏暮鳴——黃昏時段自發起鳴機率拉高、卻刻意低於破曉（暮鳴比正午密、
                        // 不如破曉滿林齊鳴）；破曉與黃昏互斥（不同 Phase），故依時辰擇一取機率。
                        let self_prob = if is_dawn {
                            DAWN_CHIRP_PROB
                        } else if is_dusk {
                            DUSK_CHIRP_PROB
                        } else {
                    ...
                    }
                {
                    ...
                    // ROADMAP 306：黃昏一段鳴唱拉長、卻介於平常與破曉之間（暮鳴比正午長、不如晨鳴久）。
                    let (lo, hi) = if is_dawn {
                        (DAWN_CHIRP_DURATION_MIN, DAWN_CHIRP_DURATION_MAX)
                    } else if is_dusk {
                        (DUSK_CHIRP_DURATION_MIN, DUSK_CHIRP_DURATION_MAX)
                    } else {
...
mod tests {
    ...
    /// ROADMAP 306：黃昏暮鳴——黃昏時段野鳥自發起鳴的機率拉高、一段鳴唱拉長。場上只有一隻鳥
    /// （無鄰可接鳴、純看自發機率），黃昏跑同樣多幀，累計處於 Chirping 的幀數應**明顯多於**平常白天
    /// （機率 0.035>0.015、時長 2.0~3.5s>1.5~3.0s）。
    #[test]
    fn dusk_bird_chirps_more_than_normal_day() {
        fn chirping_frames(is_dusk: bool) -> u32 {
            let mut mgr = WildlifeManager::new();
            let mut bird = adult_at(WildlifeKind::WildBird, 5000.0, 5000.0);
            bird.id = 1;
            bird.state = WildlifeState::Wandering { target_x: 5000.0, target_y: 5000.0, wander_timer: 1.0e9 };
            ...
            let mut frames = 0u32;
            for _ in 0..8000 {
                mgr.set_dusk(is_dusk);
                mgr.tick(0.1, &[], &att, &[], false); // is_night=false（黃昏與平常皆非夜）
                let b = mgr.animals.iter().find(|x| x.id == 1).unwrap();
                if matches!(b.state, WildlifeState::Chirping { .. }) {
                    frames += 1;
                }
        ...
        let dusk = chirping_frames(true);
        let normal = chirping_frames(false);
        assert!(dusk > 0, "黃昏時野鳥應放聲暮鳴");
        assert!(normal > 0, "平常白天野鳥仍會偶爾啁啾");
        assert!(
            dusk > normal,
            "黃昏應鳴得更密更長（dusk={dusk} 幀 應 > normal={normal} 幀）"
        );
    }

    /// ROADMAP 306：黃昏暮鳴比正午密、卻不如破曉滿林齊鳴——同樣只一隻鳥純看自發機率，破曉累計
    /// Chirping 幀數應 **多於** 黃昏（DAWN_CHIRP_PROB 0.05 > DUSK_CHIRP_PROB 0.035、晨鳴時長亦更長）。
    #[test]
    fn dawn_bird_chirps_more_than_dusk() {
        fn chirping_frames(dawn: bool, dusk: bool) -> u32 {
            let mut mgr = WildlifeManager::new();
            let mut bird = adult_at(WildlifeKind::WildBird, 5000.0, 5000.0);
            bird.id = 1;
            bird.state = WildlifeState::Wandering { target_x: 5000.0, target_y: 5000.0, wander_timer: 1.0e9 };
        ...
        let dawn = chirping_frames(true, false);
        let dusk = chirping_frames(false, true);
        assert!(dusk > 0, "黃昏時野鳥應放聲暮鳴");
        assert!(
            dawn > dusk,
            "破曉滿林齊鳴應比黃昏暮鳴更盛（dawn={dawn} 幀 應 > dusk={dusk} 幀）"
        );
    }

    /// ROADMAP 306：黃昏不破壞晝夜閘——即便黃昏旗標為真，夜間（is_night=true）連跑多幀仍不該啁啾。
    #[test]
    fn dusk_flag_does_not_chirp_at_night() {
        let mut mgr = WildlifeManager::new();
        let mut bird = adult_at(WildlifeKind::WildBird, 5000.0, 5000.0);
        bird.id = 1;
        mgr.animals = vec![bird];
        let att: HashMap<WildlifeKind, i32> = HashMap::new();
        for _ in 0..2000 {
            mgr.set_dusk(true); // 黃昏旗標為真
            mgr.tick(0.1, &[], &att, &[], true); // 但夜間
            let b = mgr.animals.iter().find(|x| x.id == 1).unwrap();
            assert!(!matches!(b.state, WildlifeState::Chirping { .. }), "夜間即便黃昏旗標為真仍不該啁啾");
        }
    }
    ...
    // ─── ROADMAP 294：聞味讀界（counter-marking sniff／犬科讀同類剛留下的領地記號）──────
    #[test]
    fn tick_sniff_holds_position_while_timer_remaining() {
        // 讀味進行中：原地不動（座標不變）、計時遞減、狀態維持 Sniffing。
        let mut rng = make_rng();
        let mut wolf = adult_at(WildlifeKind::WildWolf, 5000.0, 5000.0);
        wolf.state = WildlifeState::Sniffing { sniff_timer: 2.0 };
        wolf.tick_sniff(0.1, &mut rng);
        match wolf.state {
            WildlifeState::Sniffing { sniff_timer } => {
                assert!((sniff_timer - 1.9).abs() < 1e-4, "計時應遞減 dt");
            }
            _ => panic!("讀味未到期應維持 Sniffing，實際 {:?}", wolf.state),
        }
        assert!((wolf.x - 5000.0).abs() < 1e-6 && (wolf.y - 5000.0).abs() < 1e-6, "讀味中應原地不動");
    }

    #[test]
    fn tick_sniff_returns_to_wander_when_timer_expires() {
        // 讀味到期：起身回到巡遊。
        let mut rng = make_rng();
        let mut fox = adult_at(WildlifeKind::WildFox, 5000.0, 5000.0);
        fox.state = WildlifeState::Sniffing { sniff_timer: 0.05 };
        fox.tick_sniff(0.1, &mut rng); // dt > 剩餘 → 到期
        assert!(matches!(fox.state, WildlifeState::Wandering { .. }), "讀味到期應回巡遊，實際 {:?}", fox.state);
    }

    #[test]
    fn tick_sniff_noop_on_other_state() {
        // 防呆：非 Sniffing 狀態呼叫 tick_sniff 不該有任何作用（狀態不變）。
        let mut rng = make_rng();
        let mut wolf = adult_at(WildlifeKind::WildWolf, 5000.0, 5000.0);
        wolf.state = WildlifeState::Resting { rest_timer: 3.0 };
        wolf.tick_sniff(0.1, &mut rng);
        assert!(matches!(wolf.state, WildlifeState::Resting { .. }), "非讀味狀態呼叫 tick_sniff 不該改狀態");
    }

    #[test]
    fn sniffing_state_str_is_sniffing() {
        let mut fox = adult_at(WildlifeKind::WildFox, 0.0, 0.0);
        fox.state = WildlifeState::Sniffing { sniff_timer: 1.0 };
        assert_eq!(fox.state_str(), "sniffing");
    }

    #[test]
    fn prey_never_sniffs_a_mark() {
        // 物種專屬：讀味只發生在掠食者 phase——平靜獵物即便身邊有同類掠食者剛留下記號，連跑數百幀都不該進入
        // Sniffing。把鹿放在 FLEE_RADIUS(180) 外、SNIFF_SEE_RADIUS(220) 內（200px），證明牠不沾讀味是「物種
        // 不對」而非「離記號太遠」。
        let mut mgr = WildlifeManager::new();
        let mut fox = adult_at(WildlifeKind::WildFox, 5000.0, 5000.0); // 持續標記的吸引源（野狐不獵鹿）
        fox.id = 1;
        fox.state = WildlifeState::Marking { mark_timer: 1.0e9 };
        let mut deer = adult_at(WildlifeKind::WildDeer, 5200.0, 5000.0);
        deer.id = 2;
        deer.state = WildlifeState::Resting { rest_timer: 0.1 };
        mgr.animals = vec![fox, deer];
        let att: HashMap<WildlifeKind, i32> = HashMap::new();
        for _ in 0..300 {
            mgr.tick(0.1, &[], &att, &[], false);
            let d = mgr.animals.iter().find(|x| x.id == 2).unwrap();
            assert!(!matches!(d.state, WildlifeState::Sniffing { .. }), "獵物不該讀味，實際 {:?}", d.state);
        }
    }

    #[test]
    fn predator_does_not_sniff_without_a_marking_peer() {
        // 需附近真有同類在留味才起意：孤身野狐（場上沒有任何正在 Marking 的同類）連跑多幀都不該進入 Sniffing。
        let mut mgr = WildlifeManager::new();
        let mut fox = adult_at(WildlifeKind::WildFox, 6000.0, 6000.0);
        fox.id = 1;
        fox.state = WildlifeState::Resting { rest_timer: 0.1 };
        ...
        for _ in 0..1000 {
            mgr.tick(0.1, &[], &att, &[], false);
            let f = mgr.animals.iter().find(|x| x.id == 1).unwrap();
            assert!(!matches!(f.state, WildlifeState::Sniffing { .. }), "無同類留味時不該讀味");
        }
    }

    #[test]
    fn calm_predator_eventually_sniffs_a_fresh_mark() {
        // 白天無獵可追：一隻平靜野狐身邊有另一隻同類正留下一記新鮮記號（SNIFF_SEE_RADIUS 內持續 Marking）、
        // 無獵物——連跑多幀後牠應有機會湊上前讀這記界味（進入 Sniffing）。用同為野狐避開「野狐避狼(260)／群狼
        // 問安(287)」等其他鄰里反應的干擾。
        let mut mgr = WildlifeManager::new();
        let mut marker = adult_at(WildlifeKind::WildFox, 5000.0, 5000.0);
        marker.id = 1;
        marker.state = WildlifeState::Marking { mark_timer: 1.0e9 }; // 持續留味當吸引源
        let mut reader = adult_at(WildlifeKind::WildFox, 5100.0, 5000.0); // 100px < SNIFF_SEE_RADIUS(220)
        reader.id = 2;
        reader.state = WildlifeState::Resting { rest_timer: 0.1 };
        mgr.animals = vec![marker, reader];
        let att: HashMap<WildlifeKind, i32> = HashMap::new();
        let mut sniffed = false;
        for _ in 0..3000 {
            mgr.tick(0.1, &[], &att, &[], false); // 白天
            let r = mgr.animals.iter().find(|x| x.id == 2).unwrap();
            if matches!(r.state, WildlifeState::Sniffing { .. }) {
                sniffed = true;
                break;
            }
        }
        assert!(sniffed, "白天平靜、身邊有同類留下新鮮記號的野狐應偶爾湊上前讀味");
    }

    #[test]
    fn predator_does_not_sniff_at_night() {
        // 晝夜區隔：讀味只在白天起意（夜間掠食者改走夜嚎分支）——夜裡即便身邊有同類在留味，連跑多幀都不該
        // 進入 Sniffing。
        let mut mgr = WildlifeManager::new();
        let mut marker = adult_at(WildlifeKind::WildFox, 5000.0, 5000.0);
        marker.id = 1;
        marker.state = WildlifeState::Marking { mark_timer: 1.0e9 };
        let mut reader = adult_at(WildlifeKind::WildFox, 5100.0, 5000.0);
        reader.id = 2;
        reader.state = WildlifeState::Resting { rest_timer: 0.1 };
        mgr.animals = vec![marker, reader];
        let att: HashMap<WildlifeKind, i32> = HashMap::new();
        for _ in 0..600 {
            mgr.tick(0.1, &[], &att, &[], true); // 夜間
            let r = mgr.animals.iter().find(|x| x.id == 2).unwrap();
            assert!(!matches!(r.state, WildlifeState::Sniffing { .. }), "夜間掠食者不該讀味，實際 {:?}", r.state);
        }
    }

    #[test]
    fn sniffing_predator_wakes_to_hunt_nearby_prey() {
        // 讀味永遠讓位狩獵：讀味中的野狐一旦有可獵的小動物進入搜尋範圍，立刻改去狩獵（非繼續讀）。
        let mut mgr = WildlifeManager::new();
        let mut critter = adult_at(WildlifeKind::SmallCritter, 5120.0, 5000.0); // 120px < POUNCE_RANGE(200)
        critter.id = 100;
        critter.state = WildlifeState::Resting { rest_timer: 100.0 };
        let mut fox = adult_at(WildlifeKind::WildFox, 5000.0, 5000.0);
        fox.id = 101;
        fox.state = WildlifeState::Sniffing { sniff_timer: 1.0e9 }; // 讀得正起勁
        mgr.animals = vec![critter, fox];
        mgr.next_animal_id = 102;
        let att: HashMap<WildlifeKind, i32> = HashMap::new();
        mgr.tick(0.1, &[], &att, &[], false);
    ...
    // ─── ROADMAP 297：雨後抖水（rain shake-off）──────────────────────────────
    #[test]
    fn tick_shake_holds_position_until_timer_expires() {
        // 抖水進行中：原地不動（座標不變）、計時遞減、狀態維持 Shaking。
        let mut rng = make_rng();
        let mut deer = adult_at(WildlifeKind::WildDeer, 5000.0, 5000.0);
        deer.state = WildlifeState::Shaking { shake_timer: 2.0 };
        deer.tick_shake(0.1, &mut rng);
        match deer.state {
            WildlifeState::Shaking { shake_timer } => {
                assert!((shake_timer - 1.9).abs() < 1e-4, "計時應遞減 dt");
            }
            _ => panic!("抖水未到期應維持 Shaking，實際 {:?}", deer.state),
        }
        assert!((deer.x - 5000.0).abs() < 1e-6 && (deer.y - 5000.0).abs() < 1e-6, "抖水應原地不動");
    }

    #[test]
    fn tick_shake_returns_to_wander_when_timer_expires() {
        // 抖夠了一陣：計時耗盡就鬆下來起身回到巡遊。
        let mut rng = make_rng();
        let mut critter = adult_at(WildlifeKind::SmallCritter, 5000.0, 5000.0);
        critter.state = WildlifeState::Shaking { shake_timer: 0.05 };
        critter.tick_shake(0.1, &mut rng); // dt > 剩餘 → 到期
        assert!(matches!(critter.state, WildlifeState::Wandering { .. }), "到期應回巡遊，實際 {:?}", critter.state);
    }

    #[test]
    fn tick_shake_noop_on_other_state() {
        // 防呆：非 Shaking 狀態呼叫 tick_shake 不該有任何作用（狀態不變）。
        let mut rng = make_rng();
        let mut deer = adult_at(WildlifeKind::WildDeer, 5000.0, 5000.0);
        deer.state = WildlifeState::Resting { rest_timer: 2.0 };
        deer.tick_shake(0.1, &mut rng);
        assert!(matches!(deer.state, WildlifeState::Resting { .. }), "非抖水狀態呼叫 tick_shake 不該改狀態");
    }

    #[test]
    fn shaking_state_str_is_shaking() {
        let mut deer = adult_at(WildlifeKind::WildDeer, 0.0, 0.0);
        deer.state = WildlifeState::Shaking { shake_timer: 1.0 };
        assert_eq!(deer.state_str(), "shaking");
    }

    #[test]
    fn calm_prey_eventually_shakes_after_rain_stops() {
        // 雨剛停：一頭平靜、四下無威脅的鹿，在雨停後的時間窗內連跑多幀應有機會停下抖水（進入 Shaking）。
        let mut mgr = WildlifeManager::new();
        let mut deer = adult_at(WildlifeKind::WildDeer, 6000.0, 6000.0);
        deer.id = 1;
        deer.state = WildlifeState::Resting { rest_timer: 0.1 };
        ...
        let att: HashMap<WildlifeKind, i32> = HashMap::new();
        let mut shook = false;
        // POST_RAIN_WINDOW(12s) / dt(0.1) ≈ 120 幀內要抖到（機率 0.05/幀，window 內期望觸發數遠大於 1）
        for _ in 0..120 {
            mgr.tick(0.1, &[], &att, &[], false); // 白天、雨已停
            let d = mgr.animals.iter().find(|x| x.id == 1).unwrap();
            if matches!(d.state, WildlifeState::Shaking { .. }) {
                shook = true;
                break;
            }
        }
        assert!(shook, "雨剛停、平靜無威脅的鹿應在抖水時間窗內偶爾停下抖水");
    }

    #[test]
    fn prey_does_not_shake_without_rain_ever() {
        // 沒淋過雨就不抖：從未下過雨（post_rain_timer 一直為 0）時，平靜的鹿連跑多幀都不該進入 Shaking。
        let mut mgr = WildlifeManager::new();
        let mut deer = adult_at(WildlifeKind::WildDeer, 6000.0, 6000.0);
        deer.id = 1;
        deer.state = WildlifeState::Resting { rest_timer: 0.1 };
        ...
        for _ in 0..1000 {
            mgr.tick(0.1, &[], &att, &[], false);
            let d = mgr.animals.iter().find(|x| x.id == 1).unwrap();
            assert!(!matches!(d.state, WildlifeState::Shaking { .. }), "沒淋過雨不該抖水，實際 {:?}", d.state);
        }
    }

    #[test]
    fn prey_does_not_shake_after_window_expires() {
        // 雨停久了（窗口耗盡）就不再抖：雨停後先空跑足以耗盡 POST_RAIN_WINDOW(12s) 的時間，
        // 此後平靜的鹿連跑多幀都不該再進入 Shaking（晴了一整天的乾獸不會無端抖水）。
        let mut mgr = WildlifeManager::new();
        let mut deer = adult_at(WildlifeKind::WildDeer, 6000.0, 6000.0);
        deer.id = 1;
        // 直接讓牠處於漫遊（避開抖水起意需要的 Resting/Wandering 也行；用 Wandering 確保不誤判）
        deer.state = WildlifeState::Wandering { target_x: 6000.0, target_y: 6000.0, wander_timer: 1.0e9 };
        mgr.animals = vec![deer];
        mgr.set_raining(true);
        mgr.set_raining(false); // 開窗
        let att: HashMap<WildlifeKind, i32> = HashMap::new();
        // 先空跑 13 秒（>12s 窗口）耗盡時間窗——途中可能抖一陣，但窗口耗盡後就該止住
        for _ in 0..130 {
            mgr.tick(0.1, &[], &att, &[], false);
        }
        ...
        for _ in 0..500 {
            mgr.tick(0.1, &[], &att, &[], false);
            let d = mgr.animals.iter().find(|x| x.id == 1).unwrap();
            assert!(!matches!(d.state, WildlifeState::Shaking { .. }), "雨停久了不該再抖水，實際 {:?}", d.state);
        }
    }

    #[test]
    fn prey_does_not_shake_while_still_raining() {
        // 與 296 區隔：雨還在下時走避雨、不走抖水——下雨期間平靜的鹿連跑多幀都不該進入 Shaking。
        let mut mgr = WildlifeManager::new();
        let mut deer = adult_at(WildlifeKind::WildDeer, 6000.0, 6000.0);
        deer.id = 1;
        deer.state = WildlifeState::Resting { rest_timer: 0.1 };
        ...
        for _ in 0..1000 {
            ...
            mgr.tick(0.1, &[], &att, &[], false);
            let d = mgr.animals.iter().find(|x| x.id == 1).unwrap();
            assert!(!matches!(d.state, WildlifeState::Shaking { .. }), "下雨中應避雨、不抖水，實際 {:?}", d.state);
        }
    }

    #[test]
    fn shaking_prey_gives_way_to_flee() {
        // 抖水永遠讓位逃命：抖水中的鹿一旦有掠食者真逼進 FLEE_RADIUS(180)，立刻中斷改全速奔逃。
        let mut mgr = WildlifeManager::new();
        let mut wolf = adult_at(WildlifeKind::WildWolf, 5000.0, 5000.0);
        wolf.id = 1;
        wolf.state = WildlifeState::Wandering { target_x: 5000.0, target_y: 5000.0, wander_timer: 100.0 };
        let mut deer = adult_at(WildlifeKind::WildDeer, 5100.0, 5000.0); // 100px < FLEE_RADIUS(180)
        deer.id = 2;
        deer.state = WildlifeState::Shaking { shake_timer: 1.0e9 }; // 抖得正起勁
        mgr.animals = vec![wolf, deer];
        mgr.next_animal_id = 3;
        let att: HashMap<WildlifeKind, i32> = HashMap::new();
        mgr.tick(0.1, &[], &att, &[], false);
    ...
    #[test]
    fn prey_does_not_shake_at_night() {
        // 晝起意：夜間草食獸另走夜眠分支——即便雨剛停，夜裡連跑多幀都不該進入 Shaking。
        let mut mgr = WildlifeManager::new();
        let mut deer = adult_at(WildlifeKind::WildDeer, 6000.0, 6000.0);
        deer.id = 1;
        deer.state = WildlifeState::Resting { rest_timer: 0.1 };
        ...
        for _ in 0..120 {
            mgr.tick(0.1, &[], &att, &[], true); // 夜間
            let d = mgr.animals.iter().find(|x| x.id == 1).unwrap();
            assert!(!matches!(d.state, WildlifeState::Shaking { .. }), "夜間草食獸不該抖水，實際 {:?}", d.state);
        }
    }
    ...
    #[test]
    fn tick_nuzzle_holds_position_until_timer_expires() {
        // 嗅花進行中：原地不動（座標不變）、計時遞減、狀態維持 Nuzzling。
        let mut rng = make_rng();
        let mut deer = adult_at(WildlifeKind::WildDeer, 5000.0, 5000.0);
        deer.state = WildlifeState::Nuzzling { nuzzle_timer: 3.0 };
        deer.tick_nuzzle(0.1, &mut rng);
        match deer.state {
            WildlifeState::Nuzzling { nuzzle_timer } => {
                assert!((nuzzle_timer - 2.9).abs() < 1e-4, "計時應遞減 dt");
            }
            _ => panic!("嗅花未到期應維持 Nuzzling，實際 {:?}", deer.state),
        }
        assert!((deer.x - 5000.0).abs() < 1e-6 && (deer.y - 5000.0).abs() < 1e-6, "嗅花應原地不動");
    }

    #[test]
    fn tick_nuzzle_returns_to_wander_when_timer_expires() {
        // 嗅夠了：計時耗盡就抬頭起身回到巡遊。
        let mut rng = make_rng();
        let mut critter = adult_at(WildlifeKind::SmallCritter, 5000.0, 5000.0);
        critter.state = WildlifeState::Nuzzling { nuzzle_timer: 0.05 };
        critter.tick_nuzzle(0.1, &mut rng); // dt > 剩餘 → 到期
        assert!(matches!(critter.state, WildlifeState::Wandering { .. }), "到期應回巡遊，實際 {:?}", critter.state);
    }

    #[test]
    fn tick_nuzzle_noop_on_other_state() {
        // 防呆：非 Nuzzling 狀態呼叫 tick_nuzzle 不該有任何作用（狀態不變）。
        let mut rng = make_rng();
        let mut deer = adult_at(WildlifeKind::WildDeer, 5000.0, 5000.0);
        deer.state = WildlifeState::Resting { rest_timer: 2.0 };
        deer.tick_nuzzle(0.1, &mut rng);
        assert!(matches!(deer.state, WildlifeState::Resting { .. }), "非嗅花狀態呼叫 tick_nuzzle 不該改狀態");
    }

    #[test]
    fn nuzzling_state_str_is_nuzzling() {
        let mut deer = adult_at(WildlifeKind::WildDeer, 0.0, 0.0);
        deer.state = WildlifeState::Nuzzling { nuzzle_timer: 1.0 };
        assert_eq!(deer.state_str(), "nuzzling");
    }

    #[test]
    fn mammal_eventually_nuzzles_in_spring() {
        // 春季白天：一頭平靜、四下無威脅的鹿，在春季連跑多幀應有機會停下嗅花（進入 Nuzzling）。
        let mut mgr = WildlifeManager::new();
        let mut deer = adult_at(WildlifeKind::WildDeer, 6000.0, 6000.0);
        deer.id = 1;
        deer.state = WildlifeState::Resting { rest_timer: 0.1 };
        ...
        for _ in 0..400 {
            ...
            let d = mgr.animals.iter().find(|x| x.id == 1).unwrap();
            if matches!(d.state, WildlifeState::Nuzzling { .. }) {
                nuzzled = true;
                break;
            }
        }
        assert!(nuzzled, "春季白天、平靜無威脅的鹿應偶爾停下嗅花");
    }

    #[test]
    fn critter_eventually_nuzzles_in_spring() {
        // 草食獸全員：小動物在春季白天平靜空檔，也應偶爾停下嗅花。
        let mut mgr = WildlifeManager::new();
        let mut critter = adult_at(WildlifeKind::SmallCritter, 6000.0, 6000.0);
        critter.id = 1;
        critter.state = WildlifeState::Resting { rest_timer: 0.1 };
        ...
        for _ in 0..400 {
            mgr.tick(0.1, &[], &att, &[], false);
            let c = mgr.animals.iter().find(|x| x.id == 1).unwrap();
            if matches!(c.state, WildlifeState::Nuzzling { .. }) {
                nuzzled = true;
                break;
            }
        }
        assert!(nuzzled, "春季白天、平靜無威脅的小動物也應偶爾停下嗅花");
    }

    #[test]
    fn mammal_does_not_nuzzle_when_not_spring() {
        // 季節守衛：非春季（is_spring 為偽）時，平靜的鹿連跑多幀都不該進入 Nuzzling。
        let mut mgr = WildlifeManager::new();
        let mut deer = adult_at(WildlifeKind::WildDeer, 6000.0, 6000.0);
        deer.id = 1;
        deer.state = WildlifeState::Resting { rest_timer: 0.1 };
        ...
        for _ in 0..1000 {
            mgr.tick(0.1, &[], &att, &[], false);
            let d = mgr.animals.iter().find(|x| x.id == 1).unwrap();
            assert!(!matches!(d.state, WildlifeState::Nuzzling { .. }), "非春季不該嗅花，實際 {:?}", d.state);
        }
    }

    #[test]
    fn bird_never_nuzzles_in_spring() {
        // 物種守衛：嗅花是草食獸專屬——野鳥即便在春季白天，連跑多幀都不該進入 Nuzzling。
        let mut mgr = WildlifeManager::new();
        let mut bird = adult_at(WildlifeKind::WildBird, 6000.0, 6000.0);
        bird.id = 1;
        bird.state = WildlifeState::Resting { rest_timer: 0.1 };
        ...
        for _ in 0..1000 {
            mgr.tick(0.1, &[], &att, &[], false);
            let b = mgr.animals.iter().find(|x| x.id == 1).unwrap();
            assert!(!matches!(b.state, WildlifeState::Nuzzling { .. }), "野鳥不該嗅花，實際 {:?}", b.state);
        }
    }

    #[test]
    fn predator_never_nuzzles_in_spring() {
        // 物種守衛（與 307 喘氣／308 哆嗦刻意不同）：嗅花只屬於草食走獸——掠食的野狼／野狐即便在春季
        // 白天無獵可追的平靜空檔，連跑多幀都不該進入 Nuzzling（嗅花是草食獸的閒情，與掠食者無關）。
        for kind in [WildlifeKind::WildWolf, WildlifeKind::WildFox] {
            let mut mgr = WildlifeManager::new();
            ...
            for _ in 0..600 {
                mgr.tick(0.1, &[], &att, &[], false);
                let p = mgr.animals.iter().find(|x| x.id == 1).unwrap();
                assert!(!matches!(p.state, WildlifeState::Nuzzling { .. }), "{:?} 不該嗅花，實際 {:?}", kind, p.state);
            }
        }
    }

    #[test]
    fn mammal_does_not_nuzzle_at_night() {
        // 晝起意：夜間哺乳獸另走夜眠分支——即便春季，夜裡連跑多幀都不該進入 Nuzzling。
        let mut mgr = WildlifeManager::new();
        let mut deer = adult_at(WildlifeKind::WildDeer, 6000.0, 6000.0);
        deer.id = 1;
        deer.state = WildlifeState::Resting { rest_timer: 0.1 };
        ...
        for _ in 0..400 {
            mgr.tick(0.1, &[], &att, &[], true); // 夜間
            let d = mgr.animals.iter().find(|x| x.id == 1).unwrap();
            assert!(!matches!(d.state, WildlifeState::Nuzzling { .. }), "夜間不該嗅花，實際 {:?}", d.state);
        }
    }

    #[test]
    fn nuzzling_mammal_gives_way_to_flee() {
        // 嗅花永遠讓位逃命：嗅花中的鹿一旦有掠食者真逼進 FLEE_RADIUS(180)，立刻中斷改全速奔逃。
        let mut mgr = WildlifeManager::new();
        let mut wolf = adult_at(WildlifeKind::WildWolf, 5000.0, 5000.0);
        wolf.id = 1;
        wolf.state = WildlifeState::Wandering { target_x: 5000.0, target_y: 5000.0, wander_timer: 100.0 };
        let mut deer = adult_at(WildlifeKind::WildDeer, 5100.0, 5000.0); // 100px < FLEE_RADIUS(180)
        deer.id = 2;
        deer.state = WildlifeState::Nuzzling { nuzzle_timer: 1.0e9 }; // 嗅得正起勁
        mgr.animals = vec![wolf, deer];
        mgr.next_animal_id = 3;
        mgr.set_spring(true);
        let att: HashMap<WildlifeKind, i32> = HashMap::new();
    ...
    // ─── ROADMAP 315：秋日囤食貼膘（autumn fattening）────────────────────────────
    #[test]
    fn tick_fatten_holds_position_until_timer_expires() {
        // 囤食進行中：原地不動（座標不變）、計時遞減、狀態維持 Fattening。
        let mut rng = make_rng();
        let mut deer = adult_at(WildlifeKind::WildDeer, 5000.0, 5000.0);
        deer.state = WildlifeState::Fattening { fatten_timer: 3.0 };
        deer.tick_fatten(0.1, &mut rng);
        match deer.state {
            WildlifeState::Fattening { fatten_timer } => {
                assert!((fatten_timer - 2.9).abs() < 1e-4, "計時應遞減 dt");
            }
            _ => panic!("囤食未到期應維持 Fattening，實際 {:?}", deer.state),
        }
        assert!((deer.x - 5000.0).abs() < 1e-6 && (deer.y - 5000.0).abs() < 1e-6, "囤食應原地不動");
    }

    #[test]
    fn tick_fatten_returns_to_wander_when_timer_expires() {
        // 吃夠了：計時耗盡就抬頭起身回到巡遊。
        let mut rng = make_rng();
        let mut critter = adult_at(WildlifeKind::SmallCritter, 5000.0, 5000.0);
        critter.state = WildlifeState::Fattening { fatten_timer: 0.05 };
        critter.tick_fatten(0.1, &mut rng); // dt > 剩餘 → 到期
        assert!(matches!(critter.state, WildlifeState::Wandering { .. }), "到期應回巡遊，實際 {:?}", critter.state);
    }

    #[test]
    fn tick_fatten_noop_on_other_state() {
        // 防呆：非 Fattening 狀態呼叫 tick_fatten 不該有任何作用（狀態不變）。
        let mut rng = make_rng();
        let mut deer = adult_at(WildlifeKind::WildDeer, 5000.0, 5000.0);
        deer.state = WildlifeState::Resting { rest_timer: 2.0 };
        deer.tick_fatten(0.1, &mut rng);
        assert!(matches!(deer.state, WildlifeState::Resting { .. }), "非囤食狀態呼叫 tick_fatten 不該改狀態");
    }

    #[test]
    fn fattening_state_str_is_fattening() {
        let mut deer = adult_at(WildlifeKind::WildDeer, 0.0, 0.0);
        deer.state = WildlifeState::Fattening { fatten_timer: 1.0 };
        assert_eq!(deer.state_str(), "fattening");
    }

    #[test]
    fn mammal_eventually_fattens_in_autumn() {
        // 秋季白天：一頭平靜、四下無威脅的鹿，在秋季連跑多幀應有機會停下囤食（進入 Fattening）。
        let mut mgr = WildlifeManager::new();
        let mut deer = adult_at(WildlifeKind::WildDeer, 6000.0, 6000.0);
        deer.id = 1;
        deer.state = WildlifeState::Resting { rest_timer: 0.1 };
        ...
        for _ in 0..400 {
            ...
            let d = mgr.animals.iter().find(|x| x.id == 1).unwrap();
            if matches!(d.state, WildlifeState::Fattening { .. }) {
                fattened = true;
                break;
            }
        }
        assert!(fattened, "秋季白天、平靜無威脅的鹿應偶爾停下囤食");
    }

    #[test]
    fn critter_eventually_fattens_in_autumn() {
        // 草食獸全員：小動物在秋季白天平靜空檔，也應偶爾停下囤食。
        let mut mgr = WildlifeManager::new();
        let mut critter = adult_at(WildlifeKind::SmallCritter, 6000.0, 6000.0);
        critter.id = 1;
        critter.state = WildlifeState::Resting { rest_timer: 0.1 };
        ...
        assert!(fattened, "秋季白天、平靜無威脅的小動物也應偶爾停下囤食");
    }

    #[test]
    fn mammal_does_not_fatten_when_not_autumn() {
        // 季節守衛：非秋季（is_autumn 為偽）時，平靜的鹿連跑多幀都不該進入 Fattening。
        let mut mgr = WildlifeManager::new();
        let mut deer = adult_at(WildlifeKind::WildDeer, 6000.0, 6000.0);
        deer.id = 1;
        deer.state = WildlifeState::Resting { rest_timer: 0.1 };
        ...
        for _ in 0..1000 {
            mgr.tick(0.1, &[], &att, &[], false);
            let d = mgr.animals.iter().find(|x| x.id == 1).unwrap();
            assert!(!matches!(d.state, WildlifeState::Fattening { .. }), "非秋季不該囤食，實際 {:?}", d.state);
        }
    }

    #[test]
    fn bird_never_fattens_in_autumn() {
        // 物種守衛：囤食是草食獸專屬——野鳥即便在秋季白天（牠另走 311 集結），連跑多幀都不該進入 Fattening。
        let mut mgr = WildlifeManager::new();
        let mut bird = adult_at(WildlifeKind::WildBird, 6000.0, 6000.0);
        bird.id = 1;
        bird.state = WildlifeState::Resting { rest_timer: 0.1 };
        ...
        for _ in 0..1000 {
            mgr.tick(0.1, &[], &att, &[], false);
            let b = mgr.animals.iter().find(|x| x.id == 1).unwrap();
            assert!(!matches!(b.state, WildlifeState::Fattening { .. }), "野鳥不該囤食，實際 {:?}", b.state);
        }
    }

    #[test]
    fn predator_never_fattens_in_autumn() {
        // 物種守衛：囤食只屬於草食走獸——掠食的野狼／野狐即便在秋季白天無獵可追的平靜空檔，
        // 連跑多幀都不該進入 Fattening（增膘過冬是草食獸的事，與掠食者無關）。
        for kind in [WildlifeKind::WildWolf, WildlifeKind::WildFox] {
            let mut mgr = WildlifeManager::new();
            let mut pred = adult_at(kind, 6000.0, 6000.0);
            pred.id = 1;
            ...
            for _ in 0..600 {
                mgr.tick(0.1, &[], &att, &[], false);
                let p = mgr.animals.iter().find(|x| x.id == 1).unwrap();
                assert!(!matches!(p.state, WildlifeState::Fattening { .. }), "{:?} 不該囤食，實際 {:?}", kind, p.state);
            }
        }
    }

    #[test]
    fn mammal_does_not_fatten_at_night() {
        // 晝起意：夜間哺乳獸另走夜眠分支——即便秋季，夜裡連跑多幀都不該進入 Fattening。
        let mut mgr = WildlifeManager::new();
        let mut deer = adult_at(WildlifeKind::WildDeer, 6000.0, 6000.0);
        deer.id = 1;
        deer.state = WildlifeState::Resting { rest_timer: 0.1 };
        ...
        for _ in 0..400 {
            mgr.tick(0.1, &[], &att, &[], true); // 夜間
            let d = mgr.animals.iter().find(|x| x.id == 1).unwrap();
            assert!(!matches!(d.state, WildlifeState::Fattening { .. }), "夜間不該囤食，實際 {:?}", d.state);
        }
    }

    #[test]
    fn fattening_mammal_gives_way_to_flee() {
        // 囤食永遠讓位逃命：囤食中的鹿一旦有掠食者真逼進 FLEE_RADIUS(180)，立刻中斷改全速奔逃。
        let mut mgr = WildlifeManager::new();
        let mut wolf = adult_at(WildlifeKind::WildWolf, 5000.0, 5000.0);
        wolf.id = 1;
        wolf.state = WildlifeState::Wandering { target_x: 5000.0, target_y: 5000.0, wander_timer: 100.0 };
        let mut deer = adult_at(WildlifeKind::WildDeer, 5100.0, 5000.0); // 100px < FLEE_RADIUS(180)
        deer.id = 2;
        deer.state = WildlifeState::Fattening { fatten_timer: 1.0e9 }; // 吃得正起勁
        mgr.animals = vec![wolf, deer];
        mgr.next_animal_id = 3;
        mgr.set_autumn(true);
        let att: HashMap<WildlifeKind, i32> = HashMap::new();
