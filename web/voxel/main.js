...
// 孩子的模樣與玩耍時光 v1：孩子體型縮放倍率，與後端 `voxel_childhood::CHILD_SCALE` 同口徑。
const RES_CHILD_SCALE = 0.62;
...
function buildResident(id, name) {
  ...
  // 孩子的模樣與玩耍時光 v1：sizeGroup 包住 body，只用來套「孩子體型較小」的整體縮放，
  // 與 animateResident 既有的表情/肢體縮放（寫在 body.scale）分層、互不覆寫；名牌/泡泡/
  // 好感度/心情指示燈掛在 group 底下、不受此縮放影響，維持一貫可讀大小。
  ...
  return {
    ...
    // 孩子的模樣與玩耍時光 v1：
    lastIsChild: false, // sig 保護，體型只在孩子/成年狀態真的改變那一刻才重設縮放
    playing: false,      // 是否正玩耍（say 以 ☆ 起頭）→ 較活潑的跳動姿態
    playPulse: 0,         // 玩耍的一次性歡快脈衝（新玩耍泡泡出現時觸發，倒數歸零）
  };
}
...
function animateResident(ent, dt, fearNear) {
  ...
  if (fearNear && !asleep) {
    ...
    headDroop = 0.28;
  } else if (ent.playPulse > 0) {
    // 孩子的模樣與玩耍時光 v1：玩耍脈衝期間——比哼歌更歡快的蹦跳＋左右擺動，雙手也跟著甩，
    // 讀起來像個真的在蹦蹦跳跳的孩子（一次性，`playPulse` 倒數歸零後回到中性姿態）。
    ent.playPulse = Math.max(0, ent.playPulse - dt);
    bobY = Math.abs(Math.sin(t * 5.5)) * 0.09;
    sway = Math.sin(t * 4.5) * 0.14;
    headTilt = Math.sin(t * 4.5) * 0.1;
    armLx = 0.9 + Math.sin(t * 5.5) * 0.4;
    armRx = 0.9 + Math.sin(t * 5.5 + Math.PI) * 0.4; // 與左手反相＝甩手跑跳感
  } else if (ent.humming) {
    // 哼歌：左右輕搖 + 上下輕彈，最歡快可愛。
...
function updateResidents(list) {
  ...
  for (const r of list) {
    ...
    if (say !== ent.lastSay) {
      ent.lastSay = say;
      if (say) {
        // 泡泡文字裡的前綴符號（♪ 哼歌 / ☆ 孩子玩耍）只是前端動畫觸發器，顯示給玩家看的
        // 泡泡文字本身把符號拿掉，讀起來乾淨（比照哼歌既有做法，統一在此收斂處理）。
        const isHum = say.charAt(0) === "♪";
        const isPlay = say.charAt(0) === "☆";
        const shown = (isHum || isPlay) ? say.slice(1) : say;
        setSpriteText(ent.bubble, shown, true); ent.bubble.visible = true;
        chatLogAppend("res", r.name, shown, r.id); // 泡泡同步進聊天窗（去重會併掉截斷版）
        // 哼歌 v1（ROADMAP 788）：say 以音符「♪」起頭＝這是一段歌聲，於頭頂生成飄浮音符。
        if (isHum) spawnHumNotes(r.x, r.y, r.z);
        // 居民表情/肢體 v1：哼歌＝持續搖擺；一般說話＝觸發一次「打招呼／道賀」揮手脈衝。
        ent.humming = isHum;
        // 孩子的模樣與玩耍時光 v1：say 以「☆」起頭＝正玩起小把戲，觸發一次歡快的跳動脈衝
        // （比照 wavePulse 同款一次性脈衝手法，倒數歸零後回到中性姿態）。
        ent.playing = isPlay;
        if (isPlay) ent.playPulse = 2.2; // 玩耍脈衝約 2.2 秒（比揮手久一點，跳得更盡興）
        else if (!isHum) ent.wavePulse = 1.4; // 揮手約 1.4 秒後收手
      }
      else { ent.bubble.visible = false; ent.humming = false; ent.playing = false; }
    }
    // 孩子的模樣與玩耍時光 v1：孩子體型比成年居民小一圈，sig 保護（`lastIsChild`）只在
    // 孩子/成年狀態真的改變那一刻才重設縮放，不必逐幀寫。舊前端收不到 `is_child`
    // （undefined→falsy）安全退回成年體型，向後相容。
    const isChild = !!r.is_child;
    if (isChild !== ent.lastIsChild) {
      ent.lastIsChild = isChild;
      const s = isChild ? RES_CHILD_SCALE : 1;
      ent.sizeGroup.scale.set(s, s, s);
    }
