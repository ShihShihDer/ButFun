...
const CONVERGE_K = 9;                 // 收斂基準率（每秒，#804 沿用）：殘差大／差網路叢發跳變時用此平滑率把跳變低通掉
const CONVERGE_STOP_K = 18;           // 停下（無方向輸入）時用較大速率快對齊（每秒，靜止悄悄歸零誤差）

// ── 混合版輸入重放對帳（hybrid input-replay reconciliation，本切片）─────────────────
// 取「重放的精準」×「收斂+上限的穩健」兩者優點。QA 離線重放實測（scripts/qa/move-qa.mjs，連 prod
// 真跑、三種網路狀況）：混合版三狀況單幀 lurch 皆 ≤3px（肉眼不可見、≈#804、遠勝純重放 15–54px）、
// 最大誤差有界 ≤60px（純重放極差網路暴衝到 420px 橡皮筋）、平均領先量 ≈純重放（精準如重放）。做法：
//   1) 用『重放』算精準目標 replayTarget：從權威座標起，依序重跑「伺服器尚未確認」的輸入
//      （seq > last_input_seq）經 wasm step_player（碰撞感知、與伺服器同源）→ 精準重建預測位置。
//   2) 誤差平滑（error-smoothing）：predBase 走自己的 live 預測（每幀 step_player 跟手、平滑不抖），
//      只把「快照到達時 predBase 與 replayTarget 的殘差（＝預測誤差）」記成 predErr，每幀攤一小步
//      消化——而非每快照『硬 set』（純重放抖動／橡皮筋的元兇）。好網路殘差極小→修正極小→抖≈0。
//   3) 每幀修正量夾上限 MAX_CORRECTION_PX：last_input_seq 叢發/落後跳變、上限封頂後的大殘差，都被
//      攤成每幀 ≤此值的小步＝不抖、不橡皮筋；且修正量 > live 前進量 → 差/極差網路把 predBase 拉回有界。
//   4) 重放有『前進距離上限』REPLAY_MAX_INPUTS（鏡像 #804 的 AUTH_EXTRAP_MAX_MS）：未確認窗暴衝
//      落後（或舊伺服器 lis=0）時，最多只重跑這麼多筆 → replayTarget 絕不飛出幾百 px（有界的根）。
// 純重放（切片2）的致命缺陷正是缺 (2)(3)(4)：硬 set＋無上限 → 差網路抖 45px、極差過度預測 666px。
// 送輸入固定 15Hz（對齊伺服器 tick），每筆一個 tick；重放用同樣的 SERVER_TICK_DT 步進，
// 確保「重放步進 = 伺服器套用」對得上。wasm 未就緒時退回 #764 場景空間簡單推+收斂後備（保命，不白屏）。
...
const INPUT_HISTORY_MAX = 120;                // 輸入歷史緩衝上限（120 筆 ≈ 8 秒；確認後即截斷）
const AUTH_EXTRAP_MAX_MS = 250;               // 重放前進距離上限的時間基準（ms）：鏡像 #804，與其外插上限同量級
// 重放上限：未確認窗再大，最多重跑這麼多筆＝把 replayTarget 前進距離夾住（鏡像 AUTH_EXTRAP_MAX_MS）。
// ≈ ceil(250ms / 66.7ms) ≈ 4 筆 ≈ 4×tick 位移，與 #804 外插 250ms 的量級對齊。
const REPLAY_MAX_INPUTS = Math.max(1, Math.round(AUTH_EXTRAP_MAX_MS / INPUT_SEND_INTERVAL_MS));
const CONVERGE_K_NEAR = 12;           // 預測誤差殘差的收斂率（每秒，QA 實測選定）：夠緊貼又不把叢發殘差一次灌進畫面
const MAX_CORRECTION_PX = 3.5;        // 每幀對帳修正上限（世界 px）：直接夾住單幀 lurch ≤此值（穩健核心；> live 前進量才能在差網路拉回有界）
...
// 混合版輸入重放對帳：輸入序號 + 歷史緩衝 + 確認序號 + replayTarget。
// 每次固定節奏（15Hz）送輸入時：seq++、把 {seq, mask, run} 推進 inputHistory。
// 收到自身快照（含 last_input_seq）時：從權威基準重放 seq > last_input_seq 的未確認輸入（capped）
// 算出精準對齊目標 replayTarget；不硬 set，predWorld 每幀朝它『自適應收斂』。丟棄已確認的歷史。
let inputSeq = 0;                  // 最新已送出的輸入序號（u32，每 tick +1）
const inputHistory = [];           // [{seq, mask, run}] 尚未被伺服器確認的輸入（序號升序）
let lastSelfAuthInputSeq = 0;      // 伺服器最新確認的輸入序號（單調遞增；防 last_input_seq 抖動回退）
let lastInputSendT = 0;            // 上次送輸入的本地時間（ms），控制 15Hz 節奏
// replayTarget＝『重放』算出的精準對齊目標（世界 px，capped）；每收一筆自身快照重算。
// has=false＝尚無重放目標（剛連線／wasm 未就緒）。ver＝版本號，每重算 +1，供對帳偵測「新快照」。
const replayTarget = { x: 0, y: 0, has: false, ver: 0 };
// predErr＝「尚未消化的預測誤差殘量」（世界 px）：新快照時記下 replayTarget−predWorld，每幀攤一小步
// （收斂×夾上限）平滑消掉，而非硬 set／每幀拔河 → 好網路抖≈0、差網路有界不橡皮筋。
const predErr = { x: 0, y: 0 };
let predErrAppliedVer = 0;          // 已據此版本 replayTarget 記下殘量的版本號
...
function handleServerMsg(msg) {
  switch (msg.type) {
    ...
    case "snapshot": {
      ...
      // 混合版輸入重放：收到含自己的快照時，從「權威基準」重放「未確認輸入」算出精準對齊目標
      // replayTarget。不在此硬 set predWorld（純重放抖動／橡皮筋的元兇）——只更新 replayTarget，
      // predWorld 每幀朝它自適應收斂（updateSelfPrediction）。重放有上限＝last_input_seq 暴衝落後
      // 也不過度預測。wasm 未就緒（初始化中／舊瀏覽器）時略過，退回 #764 場景後備（updateSelfPrediction）。
      if (latestSelfWorld && wasmPredictReady()) {
        // last_input_seq：伺服器已處理到的最新輸入序號（0 表示舊版伺服器或尚無確認）。
        // 後端用 #[serde(default)] → 欄位缺失時反序列化為 0，故前端讀 || 0 亦安全。
        const lisRaw = (meAuth && typeof meAuth.last_input_seq === "number")
          ? meAuth.last_input_seq : 0;
        // 防 last_input_seq 抖動回退：確認序號只進不退（prod 上它會抖、會卡，非乾淨遞增）→
        // 用單調遞增的 lis 截斷歷史，回退抖動不會誤刪未確認輸入、也不會重複重放已消化的輸入。
        const lis = Math.max(lastSelfAuthInputSeq, lisRaw);
        // 從權威位置起，依序重放未確認輸入（seq > lis），最多 REPLAY_MAX_INPUTS 筆（前進距離上限）。
        // 步進 dt 用 SERVER_TICK_DT（與伺服器 Player::step 同步）＋ run 倍率（與 wasm run_mult() 同源）。
        // 上限是穩健性關鍵：未確認窗暴衝（差/極差網路、舊伺服器 lis=0）時，replayTarget 不會飛出幾百 px。
        let rx = latestSelfWorld.x, ry = latestSelfWorld.y, replayed = 0;
        for (const h of inputHistory) {
          if (h.seq <= lis) continue;
          if (replayed >= REPLAY_MAX_INPUTS) break;   // 上限：未確認窗暴衝也只重跑這麼多筆
          const rdt = SERVER_TICK_DT * (h.run ? wasmRunMult() : 1);
          wasmTerrain.step_player(rx, ry, h.mask, rdt);
          rx = wasmTerrain.step_out_x();
          ry = wasmTerrain.step_out_y();
          replayed++;
        }
        // 重放後的精準位置成為新的對齊目標（predWorld 每幀朝它平滑消化殘差，不硬 set）
        replayTarget.x = rx; replayTarget.y = ry; replayTarget.has = true; replayTarget.ver++;
        // 丟棄已確認的歷史（seq ≤ lis），僅保留尚未確認的
        while (inputHistory.length && inputHistory[0].seq <= lis) inputHistory.shift();
        lastSelfAuthInputSeq = lis;
      }
...
// lastSentInput 已由切片2 輸入重放的 15Hz 固定節奏送法取代（inputSeq / lastInputSendT）
...
function updateInput() {
  ...
  // 固定 15Hz 節奏送輸入（切片2 輸入重放）：每筆帶 seq，對應一個伺服器 tick。
  // 取代舊的「僅在改變時送」——固定節奏讓「一筆輸入 = 一個 tick」的映射精準，
  // 使重放的 SERVER_TICK_DT 步進與伺服器套用的 dt 對齊（這是重放能精確重建的成敗關鍵）。
  const nowMs = (typeof performance !== "undefined" && performance.now) ? performance.now() : Date.now();
  if (nowMs - lastInputSendT >= INPUT_SEND_INTERVAL_MS && ws && ws.readyState === WebSocket.OPEN) {
    lastInputSendT = nowMs;
    inputSeq = (inputSeq + 1) >>> 0; // u32 安全遞增（達 2^32 時 wrap around 為 0）
    const mask = (inputKeys.up ? 1 : 0) | (inputKeys.down ? 2 : 0) |
                 (inputKeys.left ? 4 : 0) | (inputKeys.right ? 8 : 0);
    inputHistory.push({ seq: inputSeq, mask, run: inputKeys.run });
    // 超上限時截掉最老的一筆（確認後會縮減；僅保護極端情況，例如 RTT 極高或伺服器沉默）
...
function connect() {
  ...
  ws.onopen = () => {
    ...
    // 重連後清掉輸入歷史與序號（新連線的伺服器沒有舊 seq），下一幀 updateInput 重新送
    inputSeq = 0;
    inputHistory.length = 0;
    lastSelfAuthInputSeq = 0;
    lastInputSendT = 0;
    replayTarget.has = false;   // 重連後重放目標失效，待新連線第一筆帶 lis 的快照重算
    predErr.x = 0; predErr.y = 0; predErrAppliedVer = 0; // 清掉殘量
  };
...
function updateSelfPrediction(g, dt) {
  if (wasmPredictReady() && latestSelfWorld) {
    ...
    // 對帳（混合版・誤差平滑 error-smoothing）——predWorld 已由上面的 live step 走自己的平滑預測；
    // 這裡只把「快照到達時的預測誤差殘量」攤平消化，而非每幀硬 set／拔河：
    //  · 超大誤差（>JUMP_PX，傳送／回城／復活）：直接 snap 到原始權威，清殘量、重設 replayTarget。
    //  · 新快照（replayTarget.ver 變了）：記下殘量 predErr = replayTarget − predWorld（不硬 set）。
    //    好網路殘量極小（wasm 與伺服器同源）→ 攤掉的修正極小 → 抖≈0、且 predWorld＝精準 live 預測。
    //  · 每幀：攤一小步 predErr（收斂率 CONVERGE_K_NEAR）並夾 MAX_CORRECTION_PX → 直接限住單幀 lurch；
    //    差/極差網路 replayTarget 被上限夾在權威附近、predWorld live 跑在前 → 殘量大但每幀只攤 ≤上限的小步
    //    （修正量 > live 前進量）→ 平滑把 predWorld 拉回有界範圍＝不抖、不橡皮筋（穩如 #804）。
    //  · replayTarget 尚無（剛連線還沒帶 lis 的快照）：朝原始權威記殘量（保命，不漂）。
    const err = Math.hypot(latestSelfWorld.x - predWorld.x, latestSelfWorld.y - predWorld.y);
    selfPredErrPx = err; // debug 讀數：自身預測與最近「原始權威」的距離（px）＝領先量（≈一個 RTT 內的位移）
    if (err > RECONCILE_JUMP_PX) {
      predWorld.x = latestSelfWorld.x; predWorld.y = latestSelfWorld.y;
      replayTarget.has = false; predErr.x = 0; predErr.y = 0;
    } else {
      // 新快照 → 重記殘量（以最新精準目標為準；replayTarget 尚無則用原始權威後備）
      if (replayTarget.has && replayTarget.ver !== predErrAppliedVer) {
        predErr.x = replayTarget.x - predWorld.x;
        predErr.y = replayTarget.y - predWorld.y;
        predErrAppliedVer = replayTarget.ver;
      } else if (!replayTarget.has) {
        predErr.x = latestSelfWorld.x - predWorld.x;
        predErr.y = latestSelfWorld.y - predWorld.y;
      }
      // 攤一小步殘量：收斂×夾上限（1-exp → 跟幀率無關）
      const a = 1 - Math.exp(-dt * (moving ? CONVERGE_K_NEAR : CONVERGE_STOP_K));
      let cx = predErr.x * a, cy = predErr.y * a;
      const cmag = Math.hypot(cx, cy);
      if (cmag > MAX_CORRECTION_PX) { const s = MAX_CORRECTION_PX / cmag; cx *= s; cy *= s; }
      predWorld.x += cx; predWorld.y += cy;
      predErr.x -= cx; predErr.y -= cy;
    }
