...
const CONVERGE_K = 9;                 // 移動中：predBase 每幀平滑收斂到權威的速率（每秒；≈1/K 秒時間常數，平衡誤差≈speed/K）
...
function updateSelfPrediction(g, dt) {
  if (wasmPredictReady() && latestSelfWorld) {
    ...
    // 對帳（reconcile）——「每幀平滑連續收斂」：
    //  · 超大誤差（>JUMP_PX，傳送／回城／復活）：直接 snap 到權威（保留既有行為）。
    //  · 其餘：每一幀都讓 predBase 平滑滑向最近的自身權威座標 authority（latestSelfWorld）。
    //    收斂量 1-exp(-dt*K) ＝ 跟幀率無關；authority 每快照更新、但這裡是「每幀連續」收斂，
    //    不再像舊版那樣一筆快照一次離散猛拉 0.5 → 渲染位置不再一格一格跳 ＝ 根治抖動。
    //    移動中用 CONVERGE_K（留 ~speed/K 的小平衡誤差換即時跟手）；停下用較大的 CONVERGE_STOP_K
    //    快速歸零誤差（不殘留領先，靜止悄悄對齊）。
