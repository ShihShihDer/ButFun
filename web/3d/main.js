...
// 對帳「基準每快照校準回權威」核心調校（單位＝世界 px，與 step_player 同座標）。
// 病根（舊版，已用 ?debug=1 量到）：移動中用大死區（250px）放任預測「自由往前累積、移動中不修」，
// 結果預測一路衝到權威位置前方 ~250px（≈1.4 秒移動量）才被死區邊緣的慢拉勉強擋住＝角色永遠飄在
// 真實位置前方、停下又要漂回 250px。死區太大 + 預測不回基準＝飄。
// 新解：維持一個「預測基準」predBase（＝predWorld）。每幀用 wasm step_player 往前外插「當前這一幀
// 的輸入」（即時跟手、含碰撞、跑步同源）；但每收到一筆自己的權威快照，就把 predBase 平滑校準回權威
// （兩者只差約 1 個 server tick，視覺上看不到跳）→ 誤差永遠鎖在 ~1 tick（約一二十 px），不再衝到 250。
// 停下時再每幀額外平滑收斂到權威（靜止悄悄對齊）。超大跳躍（傳送／回城／復活）仍直接 snap。
const RECONCILE_JUMP_PX = 800;        // 超大誤差（傳送／回城／復活）→ 直接 snap 到權威（世界 px）
const RECONCILE_DEADZONE_PX = 24;     // 校準死區：誤差 < 此值就不校準（殺微抖；約半個 tick 的量；世界 px）
const RECONCILE_CALIB = 0.5;          // 每收到一筆權威快照，把 predBase 往權威拉的比例（0~1；只差 ~1 tick 故看不到跳）
const RECONCILE_STOP_RATE = 12;       // 停下（無方向輸入）時每幀額外平滑收斂到權威的速率（每秒，靜止快對齊）
...
// 「自己的權威快照」序號：每收到一筆含自己的新權威快照就 +1。預測對帳用它判斷「這幀是不是
// 有新快照進來」→ 只在新快照那一幀把 predBase 校準回權威（一筆權威不會被多個 render 幀重複拉）。
let selfAuthSeq = 0;
let lastCalibSeq = -1; // 預測端「上次已消化校準」的序號（!== selfAuthSeq ＝ 有新快照待校準）
  ...
  switch (msg.type) {
    ...
    case "snapshot": {
      ...
      if (meAuth && typeof meAuth.x === "number" && typeof meAuth.y === "number") {
        ...
        selfAuthSeq++; // 新一筆自己的權威到了 → 通知預測對帳「這幀有新快照、可校準 predBase」
      }
...
function updateSelfPrediction(g, dt) {
  if (wasmPredictReady() && latestSelfWorld) {
    ...
    if (!predWorld.has) {
      ...
      predWorld.has = true; lastCalibSeq = selfAuthSeq; // 錨定即算已消化，避免錨定當幀又校準一次
    }
    ...
    // 對帳（reconcile）——「基準每快照校準回權威」：
    //  · 超大誤差（>JUMP_PX，傳送／回城／復活）：直接 snap 到權威（保留既有行為）。
    //  · 有新權威快照（selfAuthSeq 變了）：把 predBase 往權威拉 RECONCILE_CALIB 比例（預測與權威
    //    只差約 1 個 tick，這一拉視覺看不到跳）→ 誤差永遠鎖在 ~1 tick，不會自由累積衝到 250。
    //    小於 DEADZONE 就不拉（殺微抖）。一筆權威只在它「剛到的那幀」校準一次（靠 seq 比對）。
    //  · 停下（無方向輸入）：每幀額外平滑收斂到權威（不殘留 ~1 tick 領先，靜止悄悄對齊）。
    const ex = latestSelfWorld.x - predWorld.x, ey = latestSelfWorld.y - predWorld.y;
    const err = Math.hypot(ex, ey);
    selfPredErrPx = err; // debug 讀數：自身預測與最近權威位置的距離（世界 px）→ 應穩在 ~20-40
    if (err > RECONCILE_JUMP_PX) {
      ...
      if (!moving) {
        const a = 1 - Math.exp(-dt * RECONCILE_STOP_RATE); // 1-exp → 跟幀率無關
        predWorld.x += (latestSelfWorld.x - predWorld.x) * a;
        predWorld.y += (latestSelfWorld.y - predWorld.y) * a;
      }
    }
    lastCalibSeq = selfAuthSeq; // 這筆權威已消化（下筆新快照才會再校準）
  ...
  // 對帳：同 wasm 路的「基準每快照校準回權威」（這裡基準＝g.position 場景單位，閾值由世界 px × WORLD_SCALE 換算）。
  if (selfHasAuth) {
    ...
    if (err > RECONCILE_JUMP_PX * WORLD_SCALE) {
      ...
      if (selfAuthSeq !== lastCalibSeq && err > RECONCILE_DEADZONE_PX * WORLD_SCALE) {
        g.position.x += ex * RECONCILE_CALIB; g.position.z += ez * RECONCILE_CALIB; // 新快照才校準回權威
      }
      if (!moving) {
        const a = 1 - Math.exp(-dt * RECONCILE_STOP_RATE);
        g.position.x += (selfAuthX - g.position.x) * a; g.position.z += (selfAuthZ - g.position.z) * a;
      }
    }
    lastCalibSeq = selfAuthSeq; // 這筆權威已消化
  }
}
