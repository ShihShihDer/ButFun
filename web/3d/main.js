...
// 對帳「移動中信預測、停下才收斂」核心調校（單位＝世界 px，與 step_player 同座標）。
// 為什麼這樣：自身預測（wasm step_player）已含碰撞、與伺服器同源＝準的；但權威快照永遠
// 慢一個 RTT，若每幀都把自己往（落後的）權威拉＝持續往回的拉力＝走起來「飄／被往回拖」。
// 解法：移動中設「死區」——預測與權威差距在死區內就「完全不修」（全信預測，零延遲手感）；
// 停下時才平滑收斂到權威（靜止收斂快、悄悄對齊，比照 2D game.js）。超大跳躍直接 snap。
const RECONCILE_JUMP_PX = 800;       // 超大誤差（傳送／回城／復活）→ 直接 snap 到權威（世界 px）
const RECONCILE_MOVE_DEADZONE_PX = 250; // 移動中：誤差 < 此值「完全不修」（信預測，殺掉飄移；世界 px）
const RECONCILE_MOVE_RATE = 2;       // 移動中誤差超出死區時，極慢的溫和歸位速率（每秒，避免急拉）
const RECONCILE_STOP_RATE = 6;       // 停下時平滑收斂到權威的速率（每秒，靜止快對齊）
...
function updateSelfPrediction(g, dt) {
  if (wasmPredictReady() && latestSelfWorld) {
    // 第一次（或剛切換到 wasm 預測）：以最新權威世界座標錨定 predWorld（latestSelfWorld 是世界 px）。
    if (!predWorld.has) { predWorld.x = latestSelfWorld.x; predWorld.y = latestSelfWorld.y; predWorld.has = true; }
    ...
    const moving = mask !== 0;
    if (mask) {
      // step_player 內含對角正規化＋地形碰撞；跑步倍率讀 wasm（與後端 RUN_MULT 同源），
      // 用同一個 run 旗標 → 預測加速與權威一致，跑步時不會落後被拉回（橡皮筋）。
    ...
    // 對帳（reconcile）——「移動中信預測、停下才收斂」：
    //  · 超大誤差（>JUMP_PX，傳送／回城／復活）：直接 snap 到權威（保留既有行為）。
    //  · 移動中（有方向輸入）：預測含碰撞且與伺服器同源＝準的，但權威慢一個 RTT；故設死區，
    //    誤差 < DEADZONE_PX「完全不修」（殺掉飄移＝不被往回拉）；真的偏出死區才以極慢速率溫和歸位。
    //  · 停下（無方向輸入）：才平滑收斂到權威（小誤差悄悄歸位，比照 2D game.js 靜止快收斂）。
    const ex = latestSelfWorld.x - predWorld.x, ey = latestSelfWorld.y - predWorld.y;
    const err = Math.hypot(ex, ey);
    selfPredErrPx = err; // debug 讀數：自身預測與最近權威位置的距離（世界 px）
    if (err > RECONCILE_JUMP_PX) {
      predWorld.x = latestSelfWorld.x; predWorld.y = latestSelfWorld.y;
    } else if (moving) {
      if (err > RECONCILE_MOVE_DEADZONE_PX) {
        const a = 1 - Math.exp(-dt * RECONCILE_MOVE_RATE); // 1-exp → 跟幀率無關
        predWorld.x += ex * a; predWorld.y += ey * a;
      }
      // 死區內：完全不修（信預測，零延遲、不飄）
    } else {
  ...
  // 對帳：同 wasm 路的「移動中信預測、停下才收斂」（這裡是場景單位，閾值由世界 px × WORLD_SCALE 換算）。
  if (selfHasAuth) {
    ...
    if (err > RECONCILE_JUMP_PX * WORLD_SCALE) {
      ...
      if (err > RECONCILE_MOVE_DEADZONE_PX * WORLD_SCALE) {
        const a = 1 - Math.exp(-dt * RECONCILE_MOVE_RATE);
