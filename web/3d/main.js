...
const CONVERGE_K = 9;                 // 〔wasm 未就緒時後備〕移動中收斂速率（每秒）
const CONVERGE_STOP_K = 18;           // 〔wasm 未就緒時後備〕靜止時收斂速率（每秒，快對齊）

// ── 切片2 輸入重放對帳（input-replay reconciliation）─────────────────────────
// 取代原本「朝權威外插目標收斂（AUTH_EXTRAP）」的近似法，升級成 FPS/LoL 等級的正解：
//   每筆輸入帶序號 → 伺服器回報確認序號（last_input_seq）→ 客戶端從「權威基準」把「未確認輸入」
//   依序重跑 wasm step_player → 預測精準重建，不抖、不漂，零拔河感。
// 送輸入的節奏：固定 15Hz（與伺服器 tick 對齊），每筆對應一個 tick；重放時以同樣的 dt
// 步進，確保「重放步進 = 伺服器套用」，否則重放位置對不上。
// wasm 未就緒時，退回原本的 CONVERGE_K 收斂後備（保命，不白屏）。
...
// 輸入重放對帳（切片2）：輸入序號 + 歷史緩衝 + 確認序號
// 每次固定節奏（15Hz）送輸入時：seq++、把 {seq, mask, run} 推進 inputHistory。
// 收到自身快照（含 last_input_seq）時：從權威基準重放 seq > last_input_seq 的所有歷史輸入，
// 得到精準重建的預測位置；丟棄已確認（seq ≤ last_input_seq）的歷史。
let inputSeq = 0;                  // 最新已送出的輸入序號（u32，每 tick +1）
const inputHistory = [];           // [{seq, mask, run}] 尚未被伺服器確認的輸入（序號升序）
let lastSelfAuthInputSeq = 0;      // 伺服器最新確認的輸入序號（0＝尚未收到）
...
function handleServerMsg(msg) {
  switch (msg.type) {
    ...
    case "snapshot": {
      ...
      // 輸入重放對帳（切片2）：收到含自己的快照時，從「權威基準」把「未確認輸入」依序重跑一遍。
      // wasm 未就緒（初始化中／舊瀏覽器）時略過重放，退回後備收斂路（updateSelfPrediction 裡處理）。
      if (latestSelfWorld && wasmPredictReady()) {
        // last_input_seq：伺服器已處理到的最新輸入序號（0 表示舊版伺服器或尚無確認）。
        // 後端用 #[serde(default)] → 欄位缺失時反序列化為 0，故前端讀 || 0 亦安全。
        const lis = (meAuth && typeof meAuth.last_input_seq === "number")
          ? meAuth.last_input_seq : 0;
        // 從權威位置開始，依序重放每筆「伺服器尚未確認」的輸入（seq > lis）。
        // 步進 dt 使用 SERVER_TICK_DT（與伺服器 Player::step 同步）＋ run 倍率（與 wasm run_mult() 同源）。
        let rx = latestSelfWorld.x, ry = latestSelfWorld.y;
        ...
        // 重放後的位置成為新的 predWorld 基準（取代舊的「每幀朝外插目標收斂」）
        predWorld.x = rx; predWorld.y = ry; predWorld.has = true;
      ...
      hudEl.innerHTML =
        ...
        `${isTouch ? "搖桿移動 · 跑鈕切換走/跑 · 右側拖曳轉鏡頭 · 跳鈕跳 · ⚔️鈕迎敵 · 🌱鈕種樹 · 😊鈕比表情" : "WASD 移動 · Shift 跑 · 拖曳轉鏡頭 · 空白鍵跳 · R 攻擊 · T 種樹 · E 表情"}`;
...
function updateActBtns() {
  ...
  apply("campfireBtn", campfireButtonState());
}
...
function updateSelfPrediction(g, dt) {
  if (wasmPredictReady() && latestSelfWorld) {
    ...
    // 對帳（切片2 輸入重放）：reconcile 已在快照到達時做完（handleServerMsg 裡重放未確認輸入）。
    // 這裡只剩「超大誤差」需要立即 snap——傳送／回城／復活使位置瞬移一大截，重放跑不了那麼遠。
    // 每幀量誤差供 debug 讀數：重放後理論上誤差極小（≈一個 RTT 內的位移），且無叢發週期性抖動。
    const err = Math.hypot(latestSelfWorld.x - predWorld.x, latestSelfWorld.y - predWorld.y);
    selfPredErrPx = err; // debug 讀數：重放後預測與最近權威的距離（px）；應穩定低且不再有週期叢發
    if (err > RECONCILE_JUMP_PX) {
      // 超大誤差（傳送／回城／復活）→ 直接 snap 到權威，並重設 predWorld（下一快照重新重放）
      predWorld.x = latestSelfWorld.x; predWorld.y = latestSelfWorld.y;
    }
    // 注意：不再有 CONVERGE_K 的每幀收斂——重放已精準重建預測，不需要「拔河拉回」
