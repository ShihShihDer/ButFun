...
(() => {
        ...
        if (me) {
          ...
          // 行星 HUD（ROADMAP 20）：不在故鄉時顯示目前所在星球。
          {
            ...
            if (planetEl) {
              ...
              if (planet !== "home") {
                const PLANET_NAMES = { verdant: "🌿 翠幽星", crimson: "🔴 赤焰星", void: "🌑 虛空星", aether: "🌫️ 霧醚星", origin: "🌟 星源星" };
  ...
  function drawTravelFlash(now) {
    ...
    const col = travelFlash.planet === "verdant" ? "80,255,160"
               ...
               : travelFlash.planet === "origin" ? "255,240,180"
               : "220,240,255";
    ctx.save();
  ...
  // 虛空星大氣染色：在虛空星時，畫面疊一層微弱深紫暈（宇宙深淵的黑暗靜默）。
  function drawVoidAtmosphere(now) {
    const me = myId ? players.get(myId) : null;
    if (!me || me.planet !== "void") return;
    const pulse = 0.5 + 0.5 * Math.sin(now / 3600);
  ...
  // 星源星大氣染色：在星源星時，畫面疊一層微弱金白暈（宇宙起源之光的神聖氛圍）。
  function drawOriginAtmosphere(now) {
    const me = myId ? players.get(myId) : null;
    if (!me || me.planet !== "origin") return;
    const pulse = 0.6 + 0.4 * Math.sin(now / 5000);
    const alpha = 0.10 * pulse;
    ctx.save();
  ...
  function showStarChartDialog() {
    ...
    const biomeGear = [
      ...
      // 入侵首領限定（ROADMAP 160）
      { item: "ether_overlord_blade", name: "守城戰刃", biome: "入侵首領限定", icon: "⚔️" },
      // 傳說古 Alpha 限定（ROADMAP 173）
      { item: "legendary_blade", name: "傳說戰刃", biome: "傳說古Alpha限定", icon: "🌟" },
    ];
    ...
    const footer = myPlanet === "crimson"
      ...
      : myPlanet === "void"
        ...
        : myPlanet === "aether"
          ...
          : myPlanet === "origin"
            ? `<div style="color:#ffd060;font-size:0.9em;margin-top:4px;">🌟 你目前在星源星。</div>${travelBtn}`
  ...
  // 服裝造型六套（ROADMAP 99 衣櫥）。
  // 每套 [胸甲色, 褲管色, 靴子色, 腰帶色, 鉚釘色] — 原創蒸汽龐克設計，非抄外部美術。
  const COSTUMES = [
    { body: null,      pants: "#3a2810", boots: "#3a2010", belt: "#b87333", rivet: "#b87333", name: "探險家套裝" }, // 0 預設（膚色身體）
  ...
  function drawSteampunkHero(cx, cy, dir, walk, moving, isMe, sty, hairStyle, skinTone, goggleColor, costume) {
    const COPPER = "#b87333";
    const DARK   = "#2a1c10";
    // 捏臉：skinTone 決定膚色，未設則用種族預設色。
    const skinCol = SKIN_TONES[skinTone ?? 0] || sty.body;
    // 服裝造型：覆蓋胸甲色；body=null 時退回膚色（探險家套裝風格）。
    const cs = COSTUMES[costume ?? 0] || COSTUMES[0];
    const bodyCol = cs.body || skinCol;
    const pantsCol = cs.pants;
    const bootsCol = cs.boots;
    const beltCol  = cs.belt;
    const rivetCol = cs.rivet;
    // 走路時左右腳前後交替（平滑 sin 波，幅度 ±3px）。
    const swing = (moving && !reduceMotion) ? Math.sin(walk * 2) * 3 : 0;

    // ── 靴子 ──
    ctx.fillStyle = bootsCol;
  ...
  function drawPlayer(p, camX, camY) {
    ...
    if (drewClayPlayer) {
      ...
      // 染色已移除（per-player 半透明色蓋層觀感廉價、糊，依玩家回饋拔掉）→ 直接畫原圖。
      const dw = 36, dh = 36;
      ctx.drawImage(
      ...
      if (artReady) {
        drawSteampunkHero(sx, by, dir, p.walk, p.moving, isMe, sty, p.hair_style, p.skin_tone, p.goggle_color, p.costume);
      } else {
  ...
  function render() {
    ...
    safeDraw("ancientAlpha", () => drawAncientAlpha(camX, camY));   // 傳說古 Alpha（ROADMAP 173）
  ...
  function tileKindGenJS(wx, wy) {
    ...
    // 虛空星覆蓋：VOID_ZONE_MIN_X = 38000，對齊 Rust world_core（scale 80, seed 2023）。
    // 用 else if 避免翠玉藤出現在虛空星範圍（VOID_ZONE_MIN_X ≥ VERDANT_ZONE_MIN_X）。
    if (wx >= 38000) {
      const voidN = biomeNoise(wx, wy, 80, 2023);
      if (voidN > 0.78 && h < 0.68) return "void_crystal";
    } else if (wx >= 20000) {
    ...
    // 星源星覆蓋：ORIGIN_ZONE_MAX_X = -50000，對齊 Rust world_core（scale 80, seed 3141）。
    // 星源星在最遠西方，最優先判斷（避免霧醚星邏輯觸發）。
    if (wx <= -50000) {
      const originN = biomeNoise(wx, wy, 80, 3141);
      if (originN > 0.75 && h < 0.75) return "origin_crystal";
    ...
    if (b === "meadow") {
      // 野花聚落判定：對齊 Rust tile_kind_at 的野花邏輯（scale 75, seed 222）。
      const flowerN = biomeNoise(wx, wy, 75, 222);
      if (flowerN > 0.81) {
        // 野花田：55% wild_flower，45% dirt——探索者挖進來會看到整片繽紛野花。
        if (h < 0.55) return "wild_flower";
        return "dirt";
      }
      // 普通草原：偶爾有石塊(5%)，其餘為泥土。
  ...
  function drawTerrain(camX, camY) {
    ...
    for (let j = 0; j < H; j++) {
      for (let i = 0; i < W; i++) {
        ...
        // 野花叢：露出才發出金黃色光暈，埋深處隱約嫩綠；提示「挖進野花田有種子可採」。
        if (kind === "wild_flower") {
          const exposed = up || down || left || right;
          const cx_ = sx + TS / 2, cy_ = sy + TS / 2;
          // 花瓣：以中心為基準畫 4 個小圓，呈十字排列，模擬四瓣野花形。
          const petalR = exposed ? 4 : 3;
          const petalD = exposed ? 5 : 4;
          ctx.fillStyle = exposed ? "rgba(255,210,60,0.92)" : "rgba(180,150,40,0.50)";
          for (const [dx, dy] of [[-petalD, 0], [petalD, 0], [0, -petalD], [0, petalD]]) {
            ctx.beginPath();
            ctx.arc(cx_ + dx, cy_ + dy, petalR, 0, Math.PI * 2);
            ctx.fill();
          }
          // 花心：小綠點
          ctx.fillStyle = exposed ? "rgba(80,180,60,0.90)" : "rgba(60,120,40,0.50)";
          ctx.beginPath();
          ctx.arc(cx_, cy_, exposed ? 2.5 : 2, 0, Math.PI * 2);
          ctx.fill();
          if (exposed) {
            // 金黃光暈——讓野花田在草原中散發溫暖金色光暈，玩家一眼認出「這裡不一樣」。
            const g = ctx.createRadialGradient(cx_, cy_, 1, cx_, cy_, TS * 0.6);
        ...
        // 源晶：露出才散發金白色起源光暈；提示「星源星獨有、挖取可得源晶碎片」。
        if (kind === "origin_crystal") {
          const exposed = up || down || left || right;
          const cx_ = sx + TS / 2, cy_ = sy + TS / 2;
          // 源晶：八角形輪廓，象徵宇宙八方能量匯聚。
          ctx.strokeStyle = exposed ? "rgba(255,230,120,0.95)" : "rgba(200,170,80,0.45)";
          ctx.lineWidth = exposed ? 1.8 : 1;
          ctx.beginPath();
          const r = exposed ? 5.5 : 4;
          for (let i = 0; i < 8; i++) {
          ...
          ctx.closePath(); ctx.stroke();
          // 晶核：中心金白小圓點
          ctx.fillStyle = exposed ? "rgba(255,240,200,0.90)" : "rgba(200,180,100,0.45)";
          ctx.beginPath(); ctx.arc(cx_, cy_, exposed ? 2.5 : 1.8, 0, Math.PI * 2); ctx.fill();
          if (exposed) {
            // 起源光暈——金白色光暈，宇宙起源之力讓整個星源星泛出溫暖金光。
            const g = ctx.createRadialGradient(cx_, cy_, 1, cx_, cy_, TS * 0.70);
            g.addColorStop(0, "rgba(255,230,120,0.35)");
            g.addColorStop(0.6, "rgba(240,200,80,0.14)");
            g.addColorStop(1, "rgba(220,180,60,0)");
            ctx.fillStyle = g;
            ctx.fillRect(sx, sy, TS, TS);
          }
        }
  ...
  // 傳說古 Alpha（ROADMAP 173）：世界頭目，比巢穴 Alpha 更大、更壯觀。
  function drawAncientAlpha(camX, camY) {
    if (!ancientAlpha) return;
    const now = performance.now();
    const sx = ancientAlpha.x - camX;
    const sy = ancientAlpha.y - camY;
    if (sx < -120 || sx > viewW + 120 || sy < -120 || sy > viewH + 120) return;

    const hpPct = ancientAlpha.hp / Math.max(1, ancientAlpha.max_hp);
    const pulse1 = 0.7 + 0.3 * Math.sin(now / 400);
    const pulse2 = 0.7 + 0.3 * Math.sin(now / 280 + 1.2);
    const pulse3 = 0.8 + 0.2 * Math.sin(now / 600 + 2.4);

    ctx.save();
    // 最外圈：金色大光環（半徑 60）
    ctx.beginPath();
    ctx.arc(sx, sy, 60 * pulse1, 0, Math.PI * 2);
    ctx.strokeStyle = `rgba(255,200,0,${(0.35 * pulse1).toFixed(3)})`;
    ctx.lineWidth = 5;
    ctx.stroke();
    // 中圈：紫色（半徑 46）
    ctx.beginPath();
    ctx.arc(sx, sy, 46 * pulse2, 0, Math.PI * 2);
    ctx.strokeStyle = `rgba(200,100,255,${(0.55 * pulse2).toFixed(3)})`;
    ctx.lineWidth = 3;
    ctx.setLineDash([8, 5]);
    ctx.stroke();
    ctx.setLineDash([]);
    // 內圈：白色（半徑 34）
    ctx.beginPath();
    ctx.arc(sx, sy, 34 * pulse3, 0, Math.PI * 2);
    ctx.strokeStyle = `rgba(255,255,220,${(0.7 * pulse3).toFixed(3)})`;
    ctx.lineWidth = 2;
    ctx.stroke();
    ...
    const bx = sx - barW / 2, by = sy - 52;
    ctx.fillStyle = "rgba(0,0,0,0.7)";
    ctx.fillRect(bx - 1, by - 1, barW + 2, barH + 2);
    const barColor = hpPct > 0.5 ? "#4c4" : hpPct > 0.25 ? "#ca4" : "#c44";
    ...
    ctx.fillRect(lx - lw / 2, ly - 13, lw, 16);
    ctx.strokeStyle = "rgba(220,100,255,0.9)";
    ctx.lineWidth = 1.5;
    ctx.strokeRect(lx - lw / 2, ly - 13, lw, 16);
  ...
  function drawWildlife(camX, camY) {
    ...
    for (const w of wildlifeList) {
      ...
      // ROADMAP 289：幼獸學吃草——依偎在母獸身邊、低頭學媽媽就地啃嫩草的哺乳幼獸（state==="mimic_graze"）
      // 頭頂浮一枚嫩芽般的 🌱，隨一口一口的節奏輕輕一頓一頓地下沉又抬起（讀起來像小傢伙跟著媽媽學吃草）。
      // 與 215 嬉戲的 💫（繞著媽媽蹦跳玩耍）對成「玩耍／學覓食」一對；與 211 成體吃草的 🌿（成體獨自覓食）
      // 區隔——幼崽學的對象就是身邊的媽媽、頭頂是嫩芽 🌱。純前端、零協議欄位：直接讀伺服器廣播的 w.state
      // （後端只在白天依偎於母獸身邊的哺乳幼獸偶爾學吃草時才給此狀態）。
      if (w.state === "mimic_graze") {
        ctx.save();
      ...
      // ROADMAP 272：成年獨立——幼獸成熟度剛長滿、長成成體的那一刻（state==="maturing"）原地舒展
      // 定格一小段「成年禮」，頭頂浮一枚輕快閃爍的 ✨，讓「幼獸長成、正式獨立加入族群」這一瞬看得到——
      // 補上 207 繁衍→208 依偎照養→（本切片）長成→270 求偶→271 守靈這條生命循環裡原本唯一無聲的
      // 「長成」一環（這之前幼獸只是無聲地由小漸大）。與 270 ❤️ 孕育前奏、271 🥀 殞落餘韻同屬生命循環
      // 的視覺信使。純前端、零協議欄位：直接讀伺服器廣播的 w.state（後端只在成熟度剛跨過 1.0 的那一刻給）。
      if (w.state === "maturing") {
        ctx.save();
      ...
      // ROADMAP 273：初生啼鳴——剛誕生的幼獸落地後的第一刻（state==="newborn"）原地蜷縮啼鳴定格一小段，
      // 頭頂浮一枚像呼吸般徐徐脹縮的 🐣，讓「一隻新生兒剛落地、怯生生地啼了一聲才起身去找媽媽」這降臨
      // 一瞬看得到——補上 207 原本「憑空出現就立刻亂跑」的無聲誕生。與 272 ✨ 成年禮對成生命兩端的蛻變
      // 一刻（誕生 🐣／成年 ✨），同屬生命循環的視覺信使。純前端、零協議欄位：直接讀伺服器廣播的
      // w.state（後端只在幼獸剛誕生、平靜的那一小段給此狀態）。
      if (w.state === "newborn") {
        ctx.save();
      ...
      // ROADMAP 251：草食獸警戒凝望——掠食者逼近到「警戒帶」、平靜獵物停下覓食抬頭僵立戒備
      // （state==="vigilant"）頭頂浮一個微微發顫、明滅不安的 😨，讓「狼還在遠處踱步、鹿群早已僵住
      // 緊盯著、一步不敢動」一眼看得到——警覺與恐懼先於奔逃。與 212 哨兵的 👀（沒有特定威脅時的
      // 例行巡視）刻意區隔：這是臨敵當下的整群凍結；與 250 狼群匯聚追對成攻防一對。純前端、零協議
      // 欄位：直接讀伺服器廣播的 w.state（後端只在掠食者落入警戒帶、平靜獵物戒備時才給此狀態）。
      if (w.state === "vigilant") {
        ctx.save();
      ...
      // ROADMAP 253：飽足歇獵——剛飽餐後躺臥歇息的掠食者（state==="sated"）頭頂浮一個緩緩呼吸般
      // 明滅的 😴，讓「狼吃飽了、懶洋洋地歇著，附近的鹿照常吃草」一眼看得到（與夜眠 💤 區隔：💤 是
      // 入夜歸巢的獵物、😴 是白天飽餐後歇息的掠食者，與 250 飢則圍獵對成「飢則獵／飽則歇」）。
      // 純前端、零協議欄位：直接讀伺服器廣播的 w.state（後端只在掠食者吃飽歇息時才給此狀態）。
      if (w.state === "sated") {
        ctx.save();
      ...
      // ROADMAP 285：晝伏蜷睡——白天無獵可追的平靜夜行掠食者（野狼／野狐，state==="dozing"）就地蜷成
      // 一團打盹補眠，頭頂浮一枚徐徐脹縮的 😪，讓「夜行的狼／狐白天本該睏，沒獵可追時就蜷起來補眠」一眼
      // 看得到。與既有兩種睡眠符號刻意區隔：💤＝入夜歸巢的晝行獵物夜眠、😴＝白天飽餐後歇息的掠食者
      //（253 飽足）、😪＝白天蜷睡補眠的夜行掠食者（本切片）；與 219 晝行獸破曉甦醒伸展 🌅 對成「晝夜鏡像」。
      // 純前端、零協議欄位：直接讀伺服器廣播的 w.state（後端只在掠食者白天無獵可追的平靜空檔才給此狀態）。
      if (w.state === "dozing") {
        ctx.save();
      ...
      // ROADMAP 287：群狼碰鼻問安——白天無獵可追、身邊有同伴的平靜野狼（state==="greeting"）轉向夥伴碰鼻
      // 致意，頭頂浮一枚像心跳般輕輕明滅的 🧡，讓「兩頭狼頭挨著頭親暱地碰鼻問安」一眼看得到——野狼不只是
      // 冷冰冰的獵手，也是有羈絆的一群夥伴。與 216 理毛 💕（獵物群的暖）對成「獵物群的暖／狼群的暖」一對；
      // 🧡 與既有諸心刻意區隔：❤️＝求偶（270）、💕＝成體互理（216）、💗＝母獸舐犢（274）、💛/🤍＝馴養親近你
      //（205）、🧡＝狼群同伴問安（本切片）。純前端、零協議欄位：直接讀伺服器廣播的 w.state（後端只在白天無獵
      // 可追、身邊有同種成年同伴的平靜野狼問安時才給此狀態）。
      if (w.state === "greeting") {
        ctx.save();
      ...
      // ROADMAP 291：掠食者舔毛理身——白天無獵可追的平靜夜行掠食者（野狼／野狐，state==="licking"）停下、
      // 低頭舔理自己的毛，頭頂浮一枚隨一舔一舔的節奏左右一頓一頓擺動的 👅，讓「狼／狐醒著理一理皮毛」一眼
      // 看得到——犬科最招牌的清潔自理。與 276 理羽 🪶（鳥低頭梳羽）／277 搔癢 🐾（走獸抬腿撓癢）湊成「自我
      // 打理三族到齊」：飛禽理羽／走獸搔癢／掠食者舔毛。與 285 蜷睡 😪（睡著補眠）刻意區隔：那是睡著的、這
      // 是醒著的清潔自理。純前端、零協議欄位：直接讀伺服器廣播的 w.state（後端只在掠食者白天無獵可追的平靜
      // 空檔才給此狀態）。
      if (w.state === "licking") {
        ctx.save();
      ...
      // ROADMAP 293：掠食者領地嗅標——白天無獵可追的平靜掠食者（野狼／野狐，state==="marking"）湊近一處
      // 低頭以一滴氣味記號標記地盤、宣示領地，頭頂浮一枚貼地低處、隨「標一下」徐徐沁出般明滅的 💧，讓
      // 「狼／狐停下來標記自己的地盤」一眼看得到——犬科最招牌的領地行為。與 225 嗅蹤 👃（鼻子貼地循別人
      // 留的氣味讀味）對成「讀味／留味」一對；與 291 舔毛 👅（清潔自理）刻意區隔：那是醒著理皮毛、這是
      // 留下領地記號。位置壓得比舔毛的 👅 更低、更貼地（標在地上），與在嘴邊的舔毛舌頭區隔開。純前端、零
      // 協議欄位：直接讀伺服器廣播的 w.state（後端只在掠食者白天無獵可追的平靜空檔才給此狀態）。
      if (w.state === "marking") {
        ctx.save();
      ...
      // ROADMAP 294：聞味讀界——白天無獵可追的平靜掠食者（野狼／野狐，state==="sniffing"）「聞到」附近有
      // 同類正留下一記新鮮的領地記號時，湊上前停下、抬頭一嗅一嗅地讀這記界味、辨認是誰的地盤，頭頂浮一枚
      // 隨一嗅一頓上下輕動的 👃，讓「狼／狐停下來讀另一隻同類剛留下的記號」一眼看得到——把 293 嗅標 💧
      //（留下記號）那筆原本自說自話的單向留味補成雙向社交（留／讀對成一對）。位置略高於頭頂（抬頭朝空中嗅，
      // 與 293 留味壓得貼地的 💧 區隔）。與 225 嗅蹤（野狼鼻子貼地循舊味跡走，靠壓低身子表現、無浮標）
      // 也區隔開：讀界是原地抬頭讀新鮮記號、二者皆會。純前端、零協議欄位：直接讀伺服器廣播的 w.state
      //（後端只在掠食者白天無獵可追、附近有同類新鮮記號的平靜空檔才給此狀態）。
      if (w.state === "sniffing") {
        ctx.save();
      ...
      // ROADMAP 295：畏味避徑——白天平靜的草食走獸（鹿／小獸，state==="skirting"）「聞到」附近有掠食者
      // 正留下一記新鮮的領地記號（卻看不見掠食者本體）時，抬頭一驚、不安地朝那記界味的反方向警覺繞走一小段，
      // 頭頂浮一枚微微發顫、明滅不安的 😟，讓「鹿嗅到狼剛留下的氣味告示、本能地避開那片地」一眼看得到——
      // 生態學的「恐懼地景」。與 251/284 警戒的 😨（看見威脅／鳥驚飛而原地僵立凝望）刻意區隔：那是定格不動、
      // 本筆是邊走邊避（後端讓牠朝反方向位移），是嗅到界味而「挪開繞走」的迴避而非僵立。把 293 掠食者留下的
      // 記號補上第三個、也是跨物種的讀者（同類讀味 294／獵物畏味 295）。純前端、零協議欄位：直接讀伺服器廣播的
      // w.state（後端只在白天、附近有掠食者新鮮記號、自身警戒帶內又無掠食者本體的平靜走獸身上才給此狀態）。
      if (w.state === "skirting") {
        ctx.save();
      ...
      // ROADMAP 297：雨後抖水——白天雨剛停時，平靜的草食獸（鹿／鳥／小獸，state==="shaking"）停下、原地用力
      // 一抖把皮毛羽毛上的水珠甩開，頭頂浮一枚被甩開、四下飛濺的 💦（高頻小幅左右急抖，演繹「渾身一抖甩水」），
      // 讓「雨停了、剛淋濕的獸群一隻隻抖開水珠」一眼看得到——與 296 避雨的 🌧️ 對成「雨中縮身／雨後抖水」一對，
      // 把生態系「應對天氣」演完雨停的後半段。純前端、零協議欄位：直接讀伺服器廣播的 w.state（後端只在白天、
      // 雨剛停的時間窗內、無威脅逼近的平靜草食獸身上才給此狀態）。
      if (w.state === "shaking") {
        ctx.save();
      ...
      // ROADMAP 298：雨後曬太陽——白天雨剛停時，平靜的草食獸（鹿／鳥／小獸，state==="basking"）抖完水後停下、
      // 攤開身子曬太陽把毛羽曬乾取暖，頭頂浮一枚徐徐脹縮、如暖意般和緩明滅的 ☀️，讓「雨停了、出太陽，剛淋濕的
      // 獸群一隻隻攤身曬乾取暖」一眼看得到——與 297 雨後抖水的 💦 對成「抖水／曬乾」一對，把同一場雨從「下雨縮身→
      // 雨停抖水→雨後曬乾取暖」演完整。純前端、零協議欄位：直接讀伺服器廣播的 w.state（後端只在白天、雨剛停的
      // 時間窗內、無威脅逼近的平靜草食獸身上才給此狀態）。
      if (w.state === "basking") {
        ctx.save();
      ...
      // ROADMAP 309：嚴寒呵氣成霧——冬季白天，哺乳獸（state==="puffing"）平靜時偶爾呼出一團看得見的白霧：
      // 頭頂浮一枚徐徐向上飄升、緩緩脹大又淡去的 💨（像一口呼出的白霧自口鼻升起、消散在冷空氣裡）。與 308
      // 哆嗦（🥶，橫向急速抖動）動態區隔：哆嗦是定點橫抖、呵氣是緩緩上飄＋淡出。只有哺乳獸（鹿/狼/狐/小獸）
      // 會呵、野鳥不呵。純前端、零協議欄位：直接讀伺服器廣播的 w.state（後端只在冬季白天平靜時才給此狀態）。
      if (w.state === "puffing") {
        ctx.save();
      ...
      // ROADMAP 310：寒冬偎暖——寒冬寒時，身邊有同伴的社交哺乳獸（state==="huddling"）朝同伴靠攏、緊挨著
      // 分享體溫：頭頂浮一枚緩緩鼓動（隨慢正弦微微脹縮，像依偎取暖時安穩的呼吸起伏）的 🤗。與 308 哆嗦
      //（🥶，孤獸獨自橫向急抖）對成「獨則顫／群則偎」一對、動態區隔：哆嗦是孤單定點的急速橫抖、偎暖是依偎
      // 同伴時和緩的脹縮。只有社交哺乳獸（鹿/小獸/狼）會偎、獨行的野狐與野鳥不偎。純前端、零協議欄位：
      // 直接讀伺服器廣播的 w.state（後端只在寒冬寒時、身邊有同種成體夥伴時才給此狀態）。
      if (w.state === "huddling") {
        ctx.save();
      ...
      // ROADMAP 311：秋日群鳥集結——秋天白天，身邊有同伴的野鳥（state==="flocking"）朝同伴飛攏、聚作
      // 一群（南遷前的集結／遷徙躁動）：頭頂浮一枚隨慢正弦輕輕擺動（像探向遠方、躁動地尋著南行的方向）的
      // 🧭。與 310 偎暖（🤗，哺乳獸冬季湊作一團分享體溫）對成「哺乳獸冬偎暖／野鳥秋集結」一對——同是社交
      // 性物種在嚴季來臨前朝同伴聚攏成群，只是哺乳獸取暖、野鳥則為南遷集結。只有野鳥會集結（哺乳獸走偎暖／
      // 喘氣）。純前端、零協議欄位：直接讀伺服器廣播的 w.state（後端只在秋天白天、身邊有同種成鳥夥伴時才給此狀態）。
      if (w.state === "flocking") {
        ctx.save();
      ...
      // ROADMAP 318：結隊同行——草食走獸（鹿／小獸）平靜時，一隻成體偶爾領頭（state==="trekking"）朝一個
      // 方向穩步前行，附近同種夥伴錯落跟上（state==="following"）、落在身後一段隨牠一道遷移，串成一支緩緩
      // 移動的隊伍。頭頂浮一枚隨慢正弦輕輕前後挪移（像邁步行進的腳步節奏）的 🐾。與 310 偎暖（🤗，朝一點
      // 聚攏後緊挨定點取暖）／311 集結（🧭，朝一點飛攏後棲止）區隔——那是聚成一團「靜止」，這是整群一道
      // 「位移」前行；也與 206 群聚（各走各的隨機漫遊、無頭頂符號）區隔：這是一隻領頭、其餘尾隨的有向隊伍。
      // 領頭與跟隨同畫 🐾（同屬一支行進的隊伍）。純前端、零協議欄位：直接讀伺服器廣播的 w.state。
      ...
      // ROADMAP 313：春日銜枝整巢——春季白天，平靜漫步的成鳥（state==="nesting"）偶爾停下、銜起一根枝條
      // 為築巢備料：頭頂浮一枚隨銜枝整巢的節奏輕輕一頓一頓上下的 🪺。與 288 啄籽（🌾，低頭覓食）／276 理羽
      //（🪶，梳自己的羽）區隔——那是覓食、那是自理，本切片是繁殖季專屬的築巢備料。與 312 哺乳獸春嗅花（🌸）
      // 對成「春天裡，獸嗅花、鳥築巢」一對。只有野鳥（成鳥）會築巢、哺乳獸另走嗅花。純前端、零協議欄位：
      // 直接讀伺服器廣播的 w.state（後端只在春季白天平靜的成鳥整巢時才給此狀態）。
      if (w.state === "nesting") {
        ctx.save();
        ...
        ctx.translate(8, -20 - nr * 3);
        ctx.fillText("🪺", 0, 0);
        ctx.restore();
      }

      // ROADMAP 314：夏日張喙納涼——盛夏正午酷暑下，野鳥（state==="gaping"）停下、張開喙垂展雙翅散熱：頭頂
      // 浮一枚急速脹縮（字級隨明滅微微放大／縮小，透出張喙急喘的急促感）的 🥵。與 307 哺乳獸喘氣（👅）對成
      //「夏日散熱：獸喘氣／鳥張喙」一對、符號區隔：哺乳獸伸舌（👅）、野鳥張喙熱臉（🥵）。只有野鳥會張喙、哺乳獸
      // 另走 307 喘氣。純前端、零協議欄位：直接讀伺服器廣播的 w.state（後端只在夏季正午酷暑的野鳥才給此狀態）。
      if (w.state === "gaping") {
        ctx.save();
      ...
      // 只有草食獸（鹿/小獸）會貼膘、掠食的狼/狐與野鳥不貼膘。與 311 集結對成「秋天裡，鳥集結、獸貼膘」一對。
      // 純前端、零協議欄位：直接讀伺服器廣播的 w.state（後端只在秋季白天平靜的草食獸囤食時才給此狀態）。
      if (w.state === "fattening") {
        ctx.save();
        ...
        ctx.translate(8, -20 - fr * 3);
        ctx.fillText("🍂", 0, 0);
        ctx.restore();
      }

      // ROADMAP 319：寒冬刨雪覓食——冬季白天，草食獸（state==="cratering"）平靜時偶爾停下、前蹄刨開覆雪、
      // 低頭啃食底下露出的枯草：頭頂浮一枚隨刨雪節奏輕輕一頓一頓地簌簌落下又飄回的 ❄️。與 315 囤食（🍂，沉穩
      // 一頓一頓上下）動態相近卻刻意換符號（❄️ vs 🍂）以區隔「冬刨雪／秋囤膘」；與 308 哆嗦（🥶，左右急速抖動）／
      // 309 呵氣（💨）等冬日禦寒姿態以「覓食 vs 保暖」＋符號區隔。只有草食獸（鹿/小獸）會刨雪覓食、掠食的狼/狐與
      // 野鳥（另走 316 蓬羽）不刨雪。與 315 囤食對成「秋囤膘／冬刨雪」一對。純前端、零協議欄位：直接讀伺服器
      // 廣播的 w.state（後端只在冬季白天平靜的草食獸刨雪覓食時才給此狀態）。
      if (w.state === "cratering") {
        ctx.save();
      ...
      // ROADMAP 321：秋日雄鹿磨角——秋季白天，成體野鹿（state==="rubbing"）平靜時偶爾停下、朝身旁矮樹低頭、
      // 將頭上鹿角一下一下反覆磨蹭去除乾枯的鹿茸、宣示即將到來的發情季：頭頂浮一枚隨磨角節奏左右一下一下輕輕
      // 蹭動（橫向往復、像角尖抵著樹幹來回磨）的 🌳。與 315 囤食（🍂，沉穩上下）／319 刨雪（❄️）／320 逐葉
      //（🍃，打旋）以動態＋符號區隔；與 224 頂角（💥）以符號＋語意區隔（對樹獨磨 vs 同伴互牴）。只有成體野鹿
      // 會磨角（幼鹿／小獸／鳥／狼狐不磨）。與 315 囤膘並列秋季野鹿兩筆：一覓食、一求偶。純前端、零協議欄位：
      // 直接讀伺服器廣播的 w.state（後端只在秋季白天平靜的成體野鹿磨角時才給此狀態）。
      if (w.state === "rubbing") {
        ctx.save();
        ...
        ctx.translate(rf * 4, -20);
        ctx.fillText("🌳", 0, 0);
        ctx.restore();
      }

      // ROADMAP 322：盛夏甩尾驅蟲——盛夏酷暑，哺乳走獸（state==="swishing"）平靜時偶爾停下、甩尾擺耳趕開
      // 擾人的蚊蠅：頭頂浮一隻被趕得急促亂竄（左右快擺＋忽上忽下）的 🪰，讀起來像驅之不散的飛蟲在打轉。
      // 與 307 喘氣（👅）並列盛夏哺乳獸兩筆：一散熱、一驅蟲；與 314 鳥張喙（🥵）以符號＋物種區隔。只有
      // 哺乳走獸（鹿／小獸）會甩尾驅蟲（鳥／狼狐不入此分支）。純前端、零協議欄位：直接讀伺服器廣播的
      // w.state（後端只在盛夏白天平靜的哺乳走獸驅蟲時才給此狀態）。
      if (w.state === "swishing") {
        ctx.save();
        ...
        ctx.translate(sf * 5, -20 + Math.cos(now / 90) * 2);
        ctx.fillText("🪰", 0, 0);
        ctx.restore();
      }
      ...
      // 自己的羽）區隔。🌾 隨一啄一啄的節奏上下一頓一頓（低頭啄下、抬頭、再啄下）。純前端、零協議欄位：直接
      // 讀伺服器廣播的 w.state（後端只在白天平靜的野鳥啄地時才給此狀態）。
      if (w.state === "pecking") {
        ctx.save();
      ...
      // ROADMAP 278：群體呵欠——白天平靜歇息時張口打個大呵欠的草食走獸（鹿／小獸，state==="yawning"）
      // 頭頂浮一枚緩緩脹放的 🥱，讓「睏意上來、張口打了個大呵欠」一眼看得到——這口呵欠還會傳染給身邊
      // 同伴。與 277 搔癢的 🐾（漫步時各顧各的自理）對成一對：歇下了打呵欠（會傳染的睏意漣漪）。純前端、
      // 零協議欄位：直接讀伺服器廣播的 w.state（後端只在白天平靜歇息的鹿／小獸打呵欠時才給此狀態）。
      if (w.state === "yawning") {
        ctx.save();
      ...
      // ROADMAP 281：棲背啄蟲——飛上一頭平靜成年野鹿的背、替牠啄食皮毛間寄生蟲的野鳥
      // （state==="cleaning"）頭頂浮一枚 🐛，讓「鳥停在鹿背上一啄一啄替牠除蟲」這幕清潔共生一眼看得
      // 到。後端已把這隻鳥的座標逐幀貼到宿主鹿的背脊上方（騎著牠移動），故鳥身本就畫在鹿背上；這裡
      // 只補上頭頂隨啄食一上一下點動的 🐛。與 276 理羽的 🪶（替自己梳羽）刻意對成「自理／跨物種互理」
      // 一對。純前端、零協議欄位：直接讀伺服器廣播的 w.state（後端只在野鳥棲背啄蟲時才給此狀態）。
      if (w.state === "cleaning") {
        ctx.save();
      ...
      // ROADMAP 282：囤糧埋藏——白天平靜漫步時就地低頭刨土、把覓得的堅果埋進土裡留作存糧的小動物
      // （state==="caching"）頭頂浮一枚 🥜，讓「松鼠刨個小坑把堅果藏起來過冬」一眼看得到——與 222 捧食
      // 啃咬的 🌰（當場吃掉）對成「吃掉／存起」一對。純前端、零協議欄位：直接讀伺服器廣播的 w.state
      //（後端只在白天平靜漫步的小動物囤糧時才給此狀態）。
      if (w.state === "caching") {
        ctx.save();
        ...
        ctx.fillText("🥜", 4, -13 - ct * 3);
        ctx.restore();
      }

      // ROADMAP 303：掠食者埋藏剩食——白天無獵可追的平靜掠食者（野狼／野狐，state==="burying"）就地低頭刨個
      // 小坑、把先前吃剩的獵肉埋藏起來留作存糧，頭頂浮一枚隨刨土在腳邊低處一上一下的 🦴，讓「狼／狐把吃不完
      // 的肉藏起來」一眼看得到——犬科最有性格的另一個覓食去向（food caching）。與 282 小動物囤糧 🥜（囤堅果
      // 過冬）刻意對成「草食囤堅果／掠食藏獵肉」一對：律動同款（隨刨土一上一下）、符號相異（🥜 堅果／🦴 獵肉）。
      // 純前端、零協議欄位：直接讀伺服器廣播的 w.state（後端只在掠食者白天無獵可追的平靜空檔才給此狀態）。
      if (w.state === "burying") {
        ctx.save();
        ...
        ctx.fillText("🦴", 4, -13 - bt * 3);
        ctx.restore();
      }

      // ROADMAP 304：掠食者刨出存糧——先前埋過存糧的平靜掠食者（野狼／野狐，state==="recovering"）白天閒檔
      // 偶爾回頭就地低頭刨開先前埋藏的小坑、把舊藏的存糧取回，頭頂同樣浮一枚隨刨土在腳邊低處一上一下的 🦴
      //（沿用 303 埋藏同一枚骨頭符號＝「藏／取」同一筆存糧），讓「狼／狐回頭把藏的肉刨出來吃」一眼看得到——
      // 把 303 埋藏只演到「存起來」的前半段補成「埋藏→取回→再埋」的完整循環。與 303 埋藏 🦴（藏起來）對成
      //「藏／取」一對：同款律動、同枚符號、相反去向。純前端、零協議欄位：直接讀伺服器廣播的 w.state（後端只在
      // 掠食者白天無獵可追、且先前埋過存糧的平靜空檔才給此狀態）。
      if (w.state === "recovering") {
        ...
        ctx.fillText("🦴", 4, -13 - rt * 3);
        ctx.restore();
      }

      // ROADMAP 283：臥嚼反芻——白天平靜歇息時臥下回嚼草料的野鹿（state==="ruminating"）頭頂浮一枚 😌，
      // 讓「鹿吃飽了歇著、嘴巴一左一右慢條斯理地反芻」這幕安詳一眼看得到——與 211 白晝吃草的 🌿（當場
      // 低頭啃）對成「進食／反芻」一對。😌 隨慢嚼節奏輕輕左右晃（嘴巴一左一右地回嚼）、徐徐明滅。純前端、
      // 零協議欄位：直接讀伺服器廣播的 w.state（後端只在白天平靜歇息的野鹿反芻時才給此狀態）。
      if (w.state === "ruminating") {
        ctx.save();
      ...
      // ROADMAP 286：甩尾驅蟲——白天平靜被飛蟲叮擾的草食走獸（野鹿／小獸，state==="shooing"）原地甩動
      // 尾巴、抖一抖把蟲趕開，頭頂浮一枚被揮開、嗡嗡左右打轉的 🪰，讓「身邊沒鳥幫忙時，走獸自己甩尾趕蟲」
      // 一眼看得到。與 281 棲背啄蟲 🐛（有鳥幫手時的跨物種除蟲）對成「有幫手／自己來」一對；與 277 搔癢 🐾
      //（撓自己的癢）區隔。純前端、零協議欄位：直接讀伺服器廣播的 w.state（後端只在白天平靜的走獸甩尾時才給此狀態）。
      if (w.state === "shooing") {
        ctx.save();
      ...
      // 嬉戲中（frolicking）改畫 💫、打盹中（napping，290）改畫 💤、避雨中（sheltering，296）改畫 🌧️、抖水中（shaking，297）改畫 💦、曬太陽中（basking，298）改畫 ☀️、飲水中（drinking，299）改畫 💧、仰望流星中（stargazing，301）改畫 🌠、喘氣中（panting，307）改畫 👅、冷顫中（shivering，308）改畫 🥶、呵氣中（puffing，309）改畫 💨、偎暖中（huddling，310）改畫 🤗、集結中（flocking，311）改畫 🧭、嗅花中（nuzzling，312）改畫 🌸、張喙散熱中（gaping，314）改畫 🥵、舒翅中（stretching，317）改畫 🪽、逐葉中（leaf_chasing，320）改畫 🍃、甩尾驅蟲中（swishing，322）改畫 🪰，這裡略過免得兩個頭頂符號疊在一起。
  ...
  function updateBagHud(inv, equippedWeapon, equippedArmor, equippedAccessory, slotCount, slotMax, decayTimers) {
    ...
    const GEAR_DESC = {
      ...
      // 傳說古 Alpha 限定武器（ROADMAP 173）
      legendary_blade: "攻擊力 +55 💫 全遊戲最強武器（傳說晶核×1＋Alpha晶石×3＋乙太×30）",
      // 住家家具（ROADMAP 155）
  ...
  function farmAtScreen(clientX, clientY) {
    ...
    // ROADMAP 173：點到傳說古 Alpha → 送 attack_ancient_alpha（伺服器驗距離）。
    if (me && ancientAlpha) {
      const rectAA = canvas.getBoundingClientRect();
      const aawx = clientX - rectAA.left + lastCam.x;
      const aawy = clientY - rectAA.top + lastCam.y;
      const ANCIENT_CLICK_R = 60 * 60;
      if ((ancientAlpha.x - aawx) ** 2 + (ancientAlpha.y - aawy) ** 2 <= ANCIENT_CLICK_R) {
        ws.send(JSON.stringify({ type: "attack_ancient_alpha" }));
        spawnTapFlash(ancientAlpha.x, ancientAlpha.y);
        return;
      }
    }
  ...
  // 衣櫥造型名稱（對應後端 COSTUMES 陣列，ROADMAP 99）
  const COSTUME_LABELS = ["探險家套裝","工匠皮甲","商人馬甲","旅人斗篷","農夫工作服","武士鎧甲"];
  // 每套造型的代表色（胸甲色；0 號用膚色代表色）
  const COSTUME_HEX    = ["#c9a24b","#6b3d1e","#2d5038","#5a1c2c","#8b7040","#505868"];
  ...
  let selCostume = myPlayer.costume      ?? 0;
  ...
  modal.style.cssText = `
    ...
    font-family:system-ui,sans-serif;font-size:14px;min-width:320px;max-width:92vw;
    box-shadow:0 4px 32px rgba(0,0,0,0.7);`;
  modal.innerHTML = `
    <div style="font-size:1.1em;font-weight:bold;margin-bottom:14px;color:#c9a24b">🎨 外觀 &amp; 衣櫥</div>
    <div style="color:#7a9fc0;font-size:0.78em;margin-bottom:10px;letter-spacing:0.05em">── 捏臉 ──</div>
    ...
    <div style="color:#7a9fc0;font-size:0.78em;margin-bottom:10px;letter-spacing:0.05em">── 衣櫥（服裝造型）──</div>
    <div style="margin-bottom:16px">
      <div id="ap-costume" style="display:flex;gap:6px;flex-wrap:wrap">${COSTUME_LABELS.map((l,i) => `
        <button data-idx="${i}" title="${l}" style="display:flex;align-items:center;gap:5px;
          padding:5px 10px;border-radius:8px;border:1px solid rgba(201,162,75,0.4);
          background:${i===selCostume?"rgba(201,162,75,0.2)":"rgba(255,255,255,0.05)"};
          color:#e0e8ff;cursor:pointer;font-size:0.82em">
          <span style="display:inline-block;width:14px;height:14px;border-radius:3px;
            background:${COSTUME_HEX[i]};border:1px solid rgba(255,255,255,0.2);flex-shrink:0"></span>
          ${l}</button>`).join("")}
      </div>
    </div>
  ...
  // 衣櫥選中高亮（帶色塊的按鈕，背景略亮）。
  function highlightCostume(selIdx) {
    modal.querySelector("#ap-costume").querySelectorAll("button").forEach(b => {
      const i = +b.dataset.idx;
      b.style.background = i === selIdx ? "rgba(201,162,75,0.2)" : "rgba(255,255,255,0.05)";
      b.style.border = i === selIdx ? "1px solid rgba(201,162,75,0.8)" : "1px solid rgba(201,162,75,0.4)";
    });
  }
  ...
  modal.querySelector("#ap-costume").addEventListener("click", e => {
    const b = e.target.closest("button"); if (!b) return;
    selCostume = +b.dataset.idx; highlightCostume(selCostume);
  });
  ...
  modal.querySelector("#ap-save").addEventListener("click", async () => {
    try {
      const res = await fetch("/api/appearance", {
        ...
        credentials: "same-origin",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ hair_style: selHat, skin_tone: selSkin, goggle_color: selGog, costume: selCostume }),
      });
      if (res.ok) {
        // 樂觀更新本機 players map，下一幀快照也會覆蓋。
        const p = players.get(myId);
        if (p) {
          p.hair_style = selHat; p.skin_tone = selSkin; p.goggle_color = selGog;
          p.costume = selCostume;
        }
        addChat("系統", `外觀已更新！造型「${COSTUME_LABELS[selCostume]}」套用中，世界上其他玩家也能看到你的新造型！`);
        modal.remove();
      } else {
