...
2. 判斷「現在這遊戲最缺的『好玩』是什麼」。鐵律:守 GDD 北極星與紀律——**一次只主攻一個薄垂直切片、不一次做大、不碰飛船/多星球/競速(那是遠期)**;經濟迴圈(乙太有產出也要有去處)、玩家擁有感、上手第一分鐘的鉤子,優先於無止盡的打磨。
   ...
   **【總監 UI 指示 2026-06-06,前端高優先】:左上角常駐文字欄(線上/乙太/生命/背包/操作說明)太佔空間。改星露谷風——常駐只留最精簡(乙太、生命小圖示+數字),其餘收進「點圖示才開」的專屬視窗(🎒背包視窗、⚙設定/操作說明…)。手機/桌面都好用、可關。純前端重排版(快照已有資料),交 frontend 線做+迭代,不必維護者接線。把這條寫進 PLAN 的 frontend 指示、列為前端當前主攻。**
   **【總監美術方向】:採集/敵人要有「動作感」+真 sprite(已有 assets/tree.png/rock.png;採集手感命中/碎屑/晃動已上線)。乙太礦/敵人還是 emoji——之後可由前端/美術線換真圖,或設美術 agent 生圖(參考 Stardew/Minecraft/RO 的手感與風格)。**
